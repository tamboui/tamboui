 ...
 * <p>
 * This demo shows how to use ToolkitEffects to apply visual effects to specific
 * elements by ID or CSS selectors.
 * <p>
 * Controls: - 1: Apply effect to header panel (by ID) - 2: Apply effect to
 * content panel (by ID) - 3: Apply effect to footer panel (by ID) - 4: Apply
 * global effect - 5: Apply effect to all panels (by CSS selector ".main-panel")
 * - 6: Apply effect to all Panel types (by type selector "Panel") - 7: Apply
 * effect to .highlight spans inside #looping-panel (descendant selector) -
 * Space: Clear all effects - q/ESC: Quit
 * <p>
 * The looping-panel also demonstrates automatic :focus pseudo-class effects -
 * when the panel is focused (Tab to it), the .focus-demo text animates
 * automatically.
 */
public class TfxToolkitDemo {
    ...
    private void run() throws Exception {
        ...
        try (ToolkitRunner r = ToolkitRunner.builder().config(config)
                .postRenderProcessor(effects.asPostRenderProcessor()).build()) {

            // Apply different looping effects to highlighted words using CSS class
            // selectors
            effects.addEffectBySelector(".looping", Fx
                    .fadeTo(Color.WHITE, Color.rgb(0xff, 0x00, 0x80), 1000, Interpolation.SineInOut)
                    .pingPong());
            effects.addEffectBySelector(".fx", Fx
                    .fadeTo(Color.WHITE, Color.rgb(0x00, 0xd9, 0xff), 1200, Interpolation.SineInOut)
                    .pingPong());
            effects.addEffectBySelector(".different", Fx
                    .fadeTo(Color.WHITE, Color.rgb(0x00, 0xff, 0x88), 1400, Interpolation.SineInOut)
                    .pingPong());
            ...
            effects.addEffectBySelector("#looping-panel:focus .focus-demo",
                    Fx.fadeTo(Color.GRAY, Color.rgb(0xff, 0x00, 0xff), 400, Interpolation.SineInOut)
                            .pingPong());
    ...
    private Element buildUI() {
        return column(
                ...
                panel("Header",
                        column(text("TFX Toolkit Integration Demo").bold().cyan(),
                                text("Effects target by ID or CSS selector").dim()))
                        .id("header").addClass("main-panel").rounded().borderColor(HEADER_COLOR)
                        .length(5).focusable().onKeyEvent(this::handleKeyEvent),

                // Main content row with content panel and looping panel
                row(
                        // Content panel - has .main-panel class
                        panel("Content",
                                column(text("Press 1-7 to apply effects:").fg(Color.WHITE),
                                        ...
                                        text("  7: Descendant (#looping-panel .highlight)")
                                                .fg(Color.GRAY),
                                        text(""), text("Status: " + statusMessage).yellow()))
                                .id("content").addClass("main-panel").rounded()
                                .borderColor(CONTENT_COLOR).fill(),
                        ...
                        markupTextArea(
                                ...
                                .title("Looping (Tab to focus)").rounded().borderColor(ACCENT_COLOR)
                                .focusedBorderColor(Color.LIGHT_YELLOW).id("looping-panel")
                                .focusable().wrapWord().length(30))
                        .fill(),

                // Footer panel - has .main-panel class
                panel("Footer",
                        row(text("Effects running: " + (effects.isRunning() ? "yes" : "no"))
                                .fg(Color.GRAY), spacer(), text("Space").bold().fg(Color.WHITE),
                                text(" Clear  ").fg(Color.GRAY),
                                text("q/ESC").bold().fg(Color.WHITE), text(" Quit").fg(Color.GRAY)))
                        .id("footer").addClass("main-panel").rounded().borderColor(FOOTER_COLOR)
                        .length(3));
    }
    ...
    private EventResult handleKeyEvent(dev.tamboui.tui.event.KeyEvent event) {
        ...
        if (event.isChar('5')) {
            ...
            effects.addEffectBySelector(".main-panel",
                    Fx.fadeToFg(ACCENT_COLOR, 400, Interpolation.SineInOut));
        ...
        if (event.isChar('6')) {
            ...
            effects.addEffectBySelector("Panel",
                    Fx.fadeToFg(Color.WHITE, 300, Interpolation.QuadOut));
        ...
        if (event.isChar('7')) {
            ...
            effects.addEffectBySelector("#looping-panel .highlight",
                    Fx.fadeTo(Color.GRAY, Color.rgb(0x00, 0xff, 0x00), 600, Interpolation.SineInOut)
                            .pingPong());
