 ...
 * <p>
 * TfxIntegration provides a non-invasive way to add visual effects to TUI
 * applications by wrapping the event handler and renderer. Effects are
 * processed during tick events and applied after the user's rendering
 * completes.
 * <p>
 * <b>Design Philosophy:</b>
 * <ul>
 * <li><b>Non-invasive:</b> Wraps existing handlers without requiring changes to
 * application code</li>
 * <li><b>Automatic timing:</b> Uses TickEvent elapsed time for consistent
 * animation</li>
 * <li><b>Forced redraws:</b> Automatically triggers redraws when effects are
 * running</li>
 * </ul>
 * <p>
 * <b>Usage:</b>
 ...
 *
 * tui.run(tfx.wrapHandler((event, runner) -> handleEvent(event)),
...
public final class TfxIntegration {
    ...
    /**
     * Last elapsed duration from tick events. Uses AtomicReference for safe
     * visibility between event handler and renderer.
     */
     ...
     * <p>
     * The effect will be applied to the entire frame area during rendering. For
     * effects that target specific areas, use {@link #addEffect(Effect, Rect)}.
     *
     * @param effect
     *            the effect to add
     */
     ...
     *            the effect to add
     * @param area
     *            the area to apply the effect to
     */
    ...
    /**
     * Wraps an event handler to capture tick timing and force redraws when effects
     * are running.
     * <p>
     * The wrapper:
     * <ul>
     ...
     * <p>
     * This is a convenience method that wraps the handler and renderer and calls
     * {@link TuiRunner#run(EventHandler, Renderer)}.
     *
     * @param runner
     *            the TUI runner
     * @param handler
     ...
     */
    public void runWith(TuiRunner runner, EventHandler handler, Renderer renderer)
            throws Exception {
