...
/**
 * Demonstrates correct rendering of wide Unicode characters (emoji, CJK,
 * fullwidth).
 * <p>
 * Verifies:
 * <ul>
...
public class UnicodeDemo {
    ...
    private static void renderDemo(Rect area, Buffer buffer) {
        java.util.List<Rect> rows = Layout.vertical().constraints(Constraint.length(5),
                Constraint.length(4), Constraint.length(4), Constraint.length(4), Constraint.fill())
                .split(area);
    ...
    private static void renderEmojiSection(Rect area, Buffer buffer) {
        Paragraph p = Paragraph.builder().text(Text.from(
                ...
                Line.from(Span.raw(
                        "\uD83D\uDD25 Fire  \uD83C\uDF89 Party  \uD83D\uDE80 Rocket  \uD83C\uDFB5 Music")),
                Line.from(Span.raw("\u2764  Heart  \u2600  Sun  \u2B50 Star  \u26A1 Zap")),
                Line.from(Span
                        .raw("Status: \uD83D\uDFE2 OK  \uD83D\uDFE1 Warn  \uD83D\uDD34 Error"))))
    ...
    private static void renderCjkSection(Rect area, Buffer buffer) {
        Paragraph p = Paragraph.builder()
                .text(Text.from(Line.from(Span.styled("CN:  ", Style.EMPTY.fg(Color.CYAN)),
                        Span.raw("\u4E16\u754C\u4F60\u597D") // 世界你好
                ), Line.from(Span.styled("JP:  ", Style.EMPTY.fg(Color.MAGENTA)),
                        Span.raw("\u3053\u3093\u306B\u3061\u306F") // こんにちは
                ), Line.from(Span.styled("KR:  ", Style.EMPTY.fg(Color.YELLOW)),
    ...
    private static void renderWrapSection(Rect area, Buffer buffer) {
        ...
        java.util.List<Rect> cols = Layout.horizontal()
                .constraints(Constraint.percentage(50), Constraint.percentage(50)).split(area);

        Paragraph charWrap = Paragraph.builder()
                .text(Text.from(
                        Line.from(Span.raw("\u4E16\u754C\u4F60\u597D\uD83D\uDD25\uD83C\uDF89"))))
                .overflow(Overflow.WRAP_CHARACTER).block(Block.builder().borders(Borders.ALL)
                        .borderType(BorderType.ROUNDED).title("CharWrap").build())
                .build();
        charWrap.render(cols.get(0), buffer);

        Paragraph wordWrap = Paragraph.builder()
                .text(Text.from(Line.from(Span.raw("Hello \u4E16\u754C and \uD83D\uDD25 fire"))))
                .overflow(Overflow.WRAP_WORD).block(Block.builder().borders(Borders.ALL)
                        .borderType(BorderType.ROUNDED).title("WordWrap").build())
                .build();
        wordWrap.render(cols.get(1), buffer);
    }

    private static void renderEllipsisSection(Rect area, Buffer buffer) {
        java.util.List<Rect> cols = Layout.horizontal().constraints(Constraint.percentage(33),
                Constraint.percentage(34), Constraint.percentage(33)).split(area);

        Paragraph ellipsisEnd = Paragraph.builder()
                .text(Text.from(Line.from(Span.raw("\u4E16\u754C\u4F60\u597D\u5566"))))
                .overflow(Overflow.ELLIPSIS).block(Block.builder().borders(Borders.ALL)
                        .borderType(BorderType.ROUNDED).title("End").build())
                .build();
        ellipsisEnd.render(cols.get(0), buffer);

        Paragraph ellipsisStart = Paragraph.builder()
                .text(Text.from(Line.from(Span.raw("\u4E16\u754C\u4F60\u597D\u5566"))))
                .overflow(Overflow.ELLIPSIS_START).block(Block.builder().borders(Borders.ALL)
                        .borderType(BorderType.ROUNDED).title("Start").build())
                .build();
        ellipsisStart.render(cols.get(1), buffer);

        Paragraph ellipsisMid = Paragraph.builder()
                .text(Text.from(Line.from(Span.raw("\u4E16\u754C\u4F60\u597D\u5566"))))
                .overflow(Overflow.ELLIPSIS_MIDDLE).block(Block.builder().borders(Borders.ALL)
                        .borderType(BorderType.ROUNDED).title("Mid").build())
                .build();
