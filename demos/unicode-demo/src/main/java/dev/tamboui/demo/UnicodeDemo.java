...
public class UnicodeDemo {
    ...
    private void render(Frame frame) {
        ...
        List<Rect> rows = Layout.vertical().constraints(Constraint.length(7), // Emoji
                Constraint.length(6), // CJK
                Constraint.length(6), // Arabic
                Constraint.length(5), // Mixed
                Constraint.length(1), // Footer
                Constraint.fill()).split(area);
    ...
    private void renderEmojiSection(Frame frame, Rect area) {
        Paragraph p = Paragraph.builder().text(Text.from(Line.from(Span.raw(
    ...
    private void renderCjkSection(Frame frame, Rect area) {
        Paragraph p = Paragraph.builder()
                ...
                ), Line.from(Span.styled("Mix: ", Style.EMPTY.fg(Color.RED)),
    ...
    private void renderArabicSection(Frame frame, Rect area) {
        Paragraph p = Paragraph.builder()
                .text(Text.from(Line.from(Span.raw("\u0645\u0631\u062D\u0628\u0627\u064B") // مرحباً
                ), Line.from(Span.raw("\u0627\u0644\u0639\u0627\u0644\u0645") // العالم
                ), Line.from(Span
                        .raw("\u0627\u0644\u0633\u0644\u0627\u0645 \u0639\u0644\u064A\u0643\u0645") // السلام
                                                                                                    // عليكم
                ), Line.from(Span.raw("\u0634\u0643\u0631\u0627\u064B \u0644\u0643") // شكراً لك
                ))).block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .title("Arabic").build())
                .build();
        frame.renderWidget(p, area);
    ...
    private void renderMixedSection(Frame frame, Rect area) {
        Paragraph p = Paragraph.builder().text(Text.from(
                ...
                Line.from(Span
                        .raw("Test: \u2705 \u30C6\u30B9\u30C8 passed \u0646\u062C\u0627\u062D")), // テスト
                                                                                                  // +
                                                                                                  // نجاح
                Line.from(Span.raw("Deploy: \uD83D\uDE80 \u90E8\u7F72\u5B8C\u4E86 \u062A\u0645")) // 部署完了
                                                                                                  // +
                                                                                                  // تم
        )).block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED).title("Mixed")
                .build()).build();
        frame.renderWidget(p, area);
    }
    ...
    private void renderFooter(Frame frame, Rect area) {
        Paragraph footer = Paragraph.builder()
                .text(Text.from(Line.from(Span.styled("q", Style.EMPTY.fg(Color.YELLOW).bold()),
