...
public class FlexDemo {
    ...
    private static final Flex[] FLEX_MODES = {Flex.START, Flex.CENTER, Flex.END, Flex.SPACE_BETWEEN,
            Flex.SPACE_AROUND, Flex.SPACE_EVENLY};

    private static final String[] EXAMPLE_NAMES = {"1:Fixed", "2:Fill", "3:Nested", "4:Toolbar"};
    ...
    private static Element renderUI() {
        ...
        return column(
                ...
                panel(() -> column(
                        row(tab(0, currentExample), tab(1, currentExample), tab(2, currentExample),
                                tab(3, currentExample)).spacing(1).length(1),
                        row(text(" Mode: ").dim(), text(currentFlex.name()).bold().yellow(),
                                text("  [←/→] change mode  [q] quit").dim()).length(1)))
                        .rounded().borderColor(Color.DARK_GRAY).length(4).focusable()
                        .onKeyEvent(event -> {
                            if (event.isLeft()) {
                                modeIndex.updateAndGet(
                                        i -> (i - 1 + FLEX_MODES.length) % FLEX_MODES.length);
                                return EventResult.HANDLED;
                            }
                            if (event.isRight()) {
                                modeIndex.updateAndGet(i -> (i + 1) % FLEX_MODES.length);
                                return EventResult.HANDLED;
                            }
                            if (event.isChar('1')) {
                                exampleIndex.set(0);
    ...
    private static Element renderFixedSizeItems(Flex flex) {
        return column(
                // Horizontal
                panel(() -> row(box("A", Color.RED), box("B", Color.GREEN), box("C", Color.BLUE),
                        box("D", Color.YELLOW), box("E", Color.CYAN)).flex(flex))
                        .title("Horizontal: 5 boxes (width=8)").rounded().borderColor(Color.BLUE)
                        .length(5),

                // Vertical
                panel(() -> column(vbar("1", Color.RED), vbar("2", Color.GREEN),
                        vbar("3", Color.BLUE)).flex(flex)).title("Vertical: 3 bars (height=3)")
    ...
    private static Element renderFixedVsGrowing(Flex flex) {
        return column(
                ...
                row(panel(() -> text("A").bold()).rounded().borderColor(Color.RED).length(10),
                ...
                row(panel(() -> text("Fixed").bold()).rounded().borderColor(Color.YELLOW)
                        .length(12),
                        panel(() -> text("Grows").bold()).rounded().borderColor(Color.CYAN).fill(),
                        panel(() -> text("Fixed").bold()).rounded().borderColor(Color.YELLOW)
                                .length(12))
                        .length(4),
                ...
                row(panel(() -> text("1x").bold()).rounded().borderColor(Color.RED).fill(1),
    ...
    private static Element renderNestedLayouts(Flex flex) {
        return row(
                ...
                panel(() -> column(
                        ...
                        .flex(flex)).title("Column flex").rounded().borderColor(Color.CYAN).fill(),

                // Center: Nested structures
                panel(() -> column(text(" Row with flex:").bold().length(1),
                        row(panel(() -> text("L")).rounded().borderColor(Color.RED).length(6),
                                panel(() -> text("M")).rounded().borderColor(Color.GREEN).length(6),
                                panel(() -> text("R")).rounded().borderColor(Color.BLUE).length(6))
                                .flex(flex).length(4),
                        text("").length(1), text(" Column with flex:").bold().length(1),
                        column(panel(() -> text("1")).rounded().borderColor(Color.YELLOW).length(3),
                                panel(() -> text("2")).rounded().borderColor(Color.MAGENTA)
                                        .length(3),
                                panel(() -> text("3")).rounded().borderColor(Color.CYAN).length(3))
                                .flex(flex).fill()))
                        .title("Mixed nesting").rounded().borderColor(Color.YELLOW).fill(),

                // Right: Deep nesting
                panel(() -> column(
                        panel(() -> row(box("A", Color.RED), box("B", Color.GREEN)).flex(flex))
                                .rounded().borderColor(Color.WHITE).length(5),

                        panel(() -> column(vbar("X", Color.CYAN), vbar("Y", Color.MAGENTA))
                                .flex(flex)).rounded().borderColor(Color.WHITE).fill()))
                        .title("Deep nesting").rounded().borderColor(Color.MAGENTA).fill());
    }
    ...
    private static Element renderToolbar(Flex flex) {
        return column(
                // Simple toolbar
                panel(() -> row(button("New", Color.GREEN), button("Open", Color.BLUE),
                        button("Save", Color.CYAN), button("Help", Color.YELLOW),
                        button("Quit", Color.RED)).flex(flex).spacing(1)).title("Simple toolbar")
                        .rounded().borderColor(Color.BLUE).length(5),

                // Status bar style
                panel(() -> row(text(" Ready").bold().green(), text(" | Line 42, Col 15").dim(),
                        text(" | UTF-8").dim()).flex(flex)).title("Status bar").rounded()
                        .borderColor(Color.GREEN).length(3),

                // Menu bar style
                panel(() -> row(menuItem("File"), menuItem("Edit"), menuItem("View"),
                        menuItem("Help")).flex(flex).spacing(2)).title("Menu bar").rounded()
                        .borderColor(Color.YELLOW).length(3),

                spacer());
    }

    private static Element box(String label, Color color) {
        return panel(() -> text(label).bold()).rounded().borderColor(color).length(8);
    }

    private static Element vbar(String label, Color color) {
        return panel(() -> text(" " + label).bold()).rounded().borderColor(color).length(3);
    }

    private static Element button(String label, Color color) {
        return panel(() -> text(" " + label + " ").bold()).rounded().borderColor(color)
