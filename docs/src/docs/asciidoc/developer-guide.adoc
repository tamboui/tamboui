...
You can export a `Buffer` to SVG, HTML, or plain/ANSI text for screenshots, documentation, or sharing. The API is fluent: start from `buffer.export()`, choose a format, optionally configure options, then write to a file, stream, or string.
...
buffer.export().svg().toFile(Path.of("output.svg"));

// Configure options via options(Consumer)
buffer.export().svg()
    .options(o -> o.title("My App").theme(ThemeColors.defaultTheme()))
    .toFile(Path.of("output.svg"));

// Export to string or bytes
String svgString = buffer.export().svg().toString();
byte[] htmlBytes = buffer.export().html().options(o -> o.inlineStyles(true)).toBytes();

// Write to an OutputStream or Writer
buffer.export().text().to(outputStream);
----
...
* `buffer.export().svg()` — SVG (scalable vector graphic)
* `buffer.export().html()` — HTML document (with embedded or external CSS)
* `buffer.export().text()` — Plain text or ANSI-styled text

To use a custom format, pass it to `as(Format)`:

[source,java]
----
buffer.export().as(Formats.SVG).toFile(path);
----
...
[source,java]
----
buffer.export().toFile(Path.of("screenshot.svg"));   // SVG
buffer.export().toFile(Path.of("page.html"));       // HTML
buffer.export().toFile(Path.of("dump.txt"));        // Plain text
buffer.export().toFile(Path.of("styled.ansi"));     // ANSI-styled text
----
...
buffer.export().crop(titleRect).svg().toFile(Path.of("title.svg"));
...
buffer.export().crop(tableAndFooter).svg().toFile(Path.of("table_and_footer.svg"));
----
...
if (area != null && !area.isEmpty()) {
    buffer.export().crop(area).svg().toFile(Path.of("panel.svg"));
}
----

=== Theme Colors

Export formats use a default foreground/background when a cell style does not specify colors (e.g. after RESET). Use `ThemeColors.defaultTheme()` for the built-in dark theme, or create your own:
...
ThemeColors dark = ThemeColors.defaultTheme();  // Rich-style dark theme

ThemeColors custom = new ThemeColors(
    new Color.Rgb(240, 240, 240),   // foreground
    new Color.Rgb(24, 24, 24)      // background
);

buffer.export().svg().options(o -> o.theme(custom)).toFile(path);
----
...
* `theme(ThemeColors)` — default foreground/background (default: `ThemeColors.defaultTheme()`)
...
----
buffer.export().svg()
    .options(o -> o.title("Dashboard").theme(ThemeColors.defaultTheme()))
...
* `theme(ThemeColors)` — default foreground/background
...
buffer.export().html().toFile(Path.of("output.html"));
...
----
// Plain text
String plain = buffer.export().text().toString();
buffer.export().text().toFile(Path.of("dump.txt"));

// ANSI-styled (e.g. for .ansi files or terminal paste)
String ansi = buffer.export().text().options(o -> o.styles(true)).toString();
buffer.export().text().options(o -> o.styles(true)).toFile(Path.of("styled.ansi"));
----
...
* ZWJ emoji like "👨‍🦲" to display as "👨?" (truncated mid-sequence)
