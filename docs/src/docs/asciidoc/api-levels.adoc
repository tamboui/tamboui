...
{project-name} uses a dedicated *UI thread model* similar to other UI frameworks.
...
When `TuiRunner.run()` starts, it marks the current thread as the UI thread.
All event handling and rendering callbacks execute on this thread.
When `run()` exits, the UI thread is cleared.

[source,java]
----
// Check if on UI thread
if (runner.isUiThread()) {
...
To safely update UI state from background threads (network callbacks, timers, async tasks), use `runOnUiThread()`:
...
If called from the UI thread, the action executes immediately.
If called from another thread, it's queued and runs during the next event loop iteration.

For actions that should always be deferred (even when on UI thread), use `runLater()`:
...
If they modify UI state, use `runOnUiThread()`:

[source,java]
----
// Action runs on scheduler thread - use runOnUiThread for UI state
runner.schedule(() -> {
    runner.runOnUiThread(() -> {
        countdown--;
    });
...
runner.scheduleRepeating(() -> {
    runner.runOnUiThread(() -> animationFrame++);
}, Duration.ofMillis(16));
----

This explicit approach gives you control - you can do background work in the scheduled action and only post the UI update portion to the UI thread.
