...
* All general framework errors extend `TerminalException` (`dev.tamboui.TerminalException`). This is a `RuntimeException`.
* I/O errors are always wrapped in `TerminalIOException` (`dev.tamboui.TerminalIOException`), to avoid having to catch `IOException` which often are non-recoverable. 
