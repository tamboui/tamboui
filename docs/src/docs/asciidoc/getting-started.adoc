...
<dependencies>
    ...
    <!-- JLine backend (one backend is required) -->
    <dependency>
        <groupId>{maven-group}</groupId>
...
dependencies {
    implementation(platform("{maven-group}:{bom-module}:{project-version}"))
    // For Toolkit DSL (recommended)
    implementation("{maven-group}:{toolkit-module}")

    // JLine backend (required)
    implementation("{maven-group}:{jline-module}")
}
----
...
//REPOS myrepo={maven-repo}
//DEPS {maven-group}:{toolkit-module}:{project-version}
//DEPS {maven-group}:{jline-module}:{project-version}
...
Gradle runs Java via https://github.com/gradle/gradle/pull/1152[a daemon that has no terminal]. You may see errors like:
...
Run & Debug does not https://youtrack.jetbrains.com/issue/IJPL-11655/Add-option-to-run-configuration-to-launch-in-terminal[reliably get a real terminal] (IntelliJ runs via Gradle/Maven in a limited environment). Workaround: build the app, then run it from the command line or IntelliJ’s built-in terminal with `java -jar` or `jbang run`.
