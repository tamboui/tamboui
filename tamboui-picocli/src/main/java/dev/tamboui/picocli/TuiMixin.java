 ...
 * <p>
 * Use this mixin to add TUI options to your existing commands without extending
 * {@link TuiCommand}.
 *
 * <pre>
 * {@code
...
public class TuiMixin {
    ...
    @Option(names = {
            "--no-alt-screen"}, description = "Disable alternate screen mode", negatable = true, defaultValue = "false")
    private boolean noAltScreen;

    @Option(names = {
            "--show-cursor"}, description = "Show cursor in TUI mode", defaultValue = "false")
    private boolean showCursor;

    @Option(names = {"--mouse"}, description = "Enable mouse capture", defaultValue = "false")
    private boolean mouseCapture;

    @Option(names = {
            "--tick-rate"}, description = "Tick rate in milliseconds for animations (0 to disable)", defaultValue = "0")
    private int tickRateMs;

    @Option(names = {
            "--poll-timeout"}, description = "Event poll timeout in milliseconds", defaultValue = "100")
    ...
    public TuiConfig toConfig() {
        TuiConfig.Builder builder = TuiConfig.builder().alternateScreen(!noAltScreen)
                .hideCursor(!showCursor).mouseCapture(mouseCapture)
