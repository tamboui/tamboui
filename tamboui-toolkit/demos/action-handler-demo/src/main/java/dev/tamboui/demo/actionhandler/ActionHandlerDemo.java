 ...
 * </ul>
 * <p>
 * Use Tab/Shift+Tab to switch focus between counters. Arrow keys modify the
 * focused counter.
 */
public class ActionHandlerDemo {
    ...
    public void run() throws Exception {
        ...
        var bindings = BindingSets.vim().toBuilder().bind(KeyTrigger.ch('c'), "clear").build();
        ...
        // Use builder - global actions are registered programmatically to capture local
        // state
        try (var runner = ToolkitRunner.builder().bindings(bindings).build()) {

            // Register global actions programmatically
            var globalHandler = new ActionHandler(bindings).on(Actions.QUIT, e -> {
            ...
            runner.run(() -> column(
                    // Title bar
                    panel(() -> row(text(" ActionHandler Demo ").bold().cyan(), spacer(),
                            text(" Tab: switch focus ").dim(), text(" arrows: change value ").dim(),
                            text(" c: clear ").dim(), text(" q: quit ").dim())).rounded()
                    ...
                    panel(() -> {
                        ...
                        var lines = eventLog.stream()
                                .map(entry -> text(entry).fg(entry.contains("Counter A")
                                        ? Color.CYAN
                                        : entry.contains("Counter B")
                                                ? Color.MAGENTA
                                                : Color.GREEN))
                                .toArray(Element[]::new);
                        return column(lines);
