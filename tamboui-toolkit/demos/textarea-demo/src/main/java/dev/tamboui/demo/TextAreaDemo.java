...
public class TextAreaDemo implements Element {
    ...
    public void render(Frame frame, Rect area, RenderContext context) {
        column(
                // Header
                panel(() -> row(text(" TextArea Demo ").bold().cyan(), spacer(),
                        text(" Lines: " + lineCount + " ").yellow(),
                        text(" Chars: " + charCount + " ").yellow(), text(" [Tab] Switch ").dim(),
                        text(" [CTRL+c] Quit ").dim())).rounded().length(3),

                // Main content
                row(
                        // Left column - Main editor
                        column(textArea(mainEditorState).title("Main Editor").showLineNumbers()
                                .rounded().focusedBorderColor(Color.CYAN).id("main-editor")
                                .onTextChange(text -> updateCounts()).fill()).percent(50),

                        // Right column - Notes and info
                        column(
                                // Notes area with placeholder
                                textArea(notesState).title("Notes")
                                        .placeholder("Type your notes here...").rounded()
                                        .focusedBorderColor(Color.CYAN).id("notes").fill(),

                                // Read-only display area (not focusable)
                                textArea(readOnlyState).title("Display (Read-only)")
                                        .showLineNumbers()
                                        .lineNumberStyle(Style.EMPTY.fg(Color.CYAN))
                                        .showCursor(false).rounded().length(8),

                                // Help panel
                                panel(() -> column(text("Keyboard Shortcuts:").bold().cyan(),
                                        text(" Arrow keys - Navigate").dim(),
                                        text(" Home/End - Line start/end").dim(),
                                        text(" Enter - New line").dim(),
                                        text(" Tab - 4 spaces").dim(),
                                        text(" Backspace/Del - Delete").dim())).title("Help")
                                        .rounded().length(9))
                                .fill())
                        .fill(),

                // Footer with current cursor position
                panel(() -> row(text("Cursor: ").dim(),
                        text("Line " + (mainEditorState.cursorRow() + 1)).cyan(), text(", ").dim(),
                        text("Col " + (mainEditorState.cursorCol() + 1)).cyan(), spacer(),
                        text("Scroll: Row " + mainEditorState.scrollRow()).dim())).rounded()
                        .length(3))
