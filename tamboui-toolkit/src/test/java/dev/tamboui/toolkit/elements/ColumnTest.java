...
class ColumnTest {
    ...
    void preferredWidth_sameWidth() {
        Column column = column(text("AAA"), // 3
                text("BBB"), // 3
                text("CCC") // 3
        );
        // All 3, so max = 3
    ...
    void preferredWidth_withMargin() {
        Column column = column(text("Hello") // 5
    ...
    void preferredWidth_withUniformMargin() {
        Column column = column(text("Test") // 4
        ).margin(1);
        // 4 + 1 (left) + 1 (right) = 6
    ...
    void preferredWidth_withTabs() {
        Column column = column(text("Short"), // 5
    ...
    void preferredWidth_withWaveText() {
        Column column = column(waveText("Loading..."), // 10
                text("Done") // 4
        );
        // Max of 10, 4 = 10
        assertThat(column.preferredWidth()).isEqualTo(10);
    ...
    void preferredWidth_withRowChildren() {
        Column column = column(row(text("A"), text("B")), // 1 + 1 = 2
    ...
    void rendersToBuffer() {
        ...
        column(text("A"), text("B"), text("C")).spacing(1).render(frame, area,
                RenderContext.empty());
    ...
    void withFlexCenter() {
        ...
        column(text("A"), text("B")).flex(Flex.CENTER).render(frame, area, RenderContext.empty());
    ...
    void fluentApiChaining() {
        Column column = column(text("A"), text("B")).spacing(1).flex(Flex.SPACE_BETWEEN).margin(2);
