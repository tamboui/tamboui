...
class ColumnsElementTest {
    ...
    void preferredWidth_multipleChildren() {
        ColumnsElement cols = columns(text("A"), // 1
                text("BB"), // 2
                text("CCC") // 3
        );
    ...
    void preferredWidth_withSpacing() {
        ColumnsElement cols = columns(text("AA"), // 2
                text("BB"), // 2
                text("CC") // 2
        ).spacing(1);
    ...
    void preferredWidth_withMargin() {
        ColumnsElement cols = columns(text("Hello") // 5
    ...
    void preferredWidth_withExplicitColumnCount() {
        ColumnsElement cols = columns(text("AAAA"), // 4
                text("BB"), // 2
                text("CCC"), // 3
                text("D") // 1
        ).columnCount(2);
    ...
    void rendersToBuffer() {
        ...
        columns(text("A"), text("B"), text("C")).spacing(1).render(frame, area,
                RenderContext.empty());

        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "A").hasSymbolAt(7, 0, "B")
                .hasSymbolAt(14, 0, "C");
    }
    ...
    void autoColumnCount() {
        // 6 items of width 5
        ColumnsElement cols = columns(text("AAAAA"), text("BBBBB"), text("CCCCC"), text("DDDDD"),
                text("EEEEE"), text("FFFFF"));
        ...
        BufferAssertions.assertThat(wideBuffer).hasSymbolAt(0, 0, "A").hasSymbolAt(5, 0, "B")
                .hasSymbolAt(10, 0, "C").hasSymbolAt(15, 0, "D").hasSymbolAt(20, 0, "E")
                .hasSymbolAt(25, 0, "F");
        ...
        BufferAssertions.assertThat(narrowBuffer).hasSymbolAt(0, 0, "A") // row 0, col 0
    ...
    void rowFirstOrdering() {
        ...
        columns(text("A"), text("B"), text("C"), text("D")).columnCount(2).render(frame, area,
                RenderContext.empty());

        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "A") // row 0, col 0
    ...
    void columnFirstOrdering() {
        ...
        columns(text("A"), text("B"), text("C"), text("D")).columnCount(2).columnFirst()
                .render(frame, area, RenderContext.empty());

        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "A") // row 0, col 0 (child 0)
    ...
    void explicitColumnCount() {
        ...
        columns(text("AAAAAAAAAA"), text("BBBBBBBBBB"), text("CCCCCCCCCC"), text("DDDDDDDDDD"))
                .columnCount(2).render(frame, area, RenderContext.empty());

        // 2 cols in 100 wide = 50 per col
        // row0=[A,B], row1=[C,D]
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "A").hasSymbolAt(50, 0, "B")
                .hasSymbolAt(0, 1, "C").hasSymbolAt(50, 1, "D");

        assertThat(columns(text("AAAAAAAAAA"), text("BBBBBBBBBB"), text("CCCCCCCCCC"),
                text("DDDDDDDDDD")).columnCount(2).preferredHeight(100, RenderContext.empty()))
                .isEqualTo(2);
    }
    ...
    void spacingBetweenColumns() {
        ...
        columns(text("AA"), text("BB"), text("CC")).spacing(2).render(frame, area,
                RenderContext.empty());
    ...
    void fluentApiChaining() {
        ColumnsElement cols = columns(text("A"), text("B")).spacing(1).flex(Flex.SPACE_BETWEEN)
                .margin(2).columnCount(2).order(ColumnOrder.COLUMN_FIRST);
    ...
    void preferredHeightSizesBufferForAllRows() {
        ...
        BufferAssertions.assertThat(truncated).hasSymbolAt(0, 0, "A").hasSymbolAt(10, 0, "B");
    ...
    class CssTests {
        ...
        void cssSpacing() {
            ...
            columns(text("AA"), text("BB"), text("CC")).addClass("spaced").render(frame, area, ctx);
        ...
        void cssMargin() {
            ...
            columns(text("A"), text("B")).addClass("margined").render(frame, area, ctx);

            // margin left=2, so content starts at x=2
            // 16 effective width, 2 cols of 8
            BufferAssertions.assertThat(buffer).hasSymbolAt(2, 0, "A").hasSymbolAt(10, 0, "B");
        }
        ...
        void cssColumnCount() {
            ...
            columns(text("A"), text("B"), text("C"), text("D")).addClass("two-cols").render(frame,
                    area, ctx);
        ...
        void cssColumnOrder() {
            ...
            columns(text("A"), text("B"), text("C"), text("D")).addClass("col-first").render(frame,
                    area, ctx);
        ...
        void programmaticOverridesCss() {
            ...
            columns(text("A"), text("B"), text("C"), text("D")).addClass("css-props").columnCount(4)
                    .order(ColumnOrder.COLUMN_FIRST).spacing(0).render(frame, area, ctx);
            ...
            BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "A").hasSymbolAt(5, 0, "B")
                    .hasSymbolAt(10, 0, "C").hasSymbolAt(15, 0, "D");
        }
    }
}
