...
/**
 * A 5-region dock layout element that arranges children into top, bottom, left,
 ...
 * 
 * <pre>
 * dock().top(text("Header").bold()).bottom(text("Footer").dim()).left(text("Sidebar"))
 *         .center(text("Main Content")).topHeight(Constraint.length(3))
...
public final class DockElement extends StyledElement<DockElement> {
     ...
     */
    public static final PropertyDefinition<Constraint> DOCK_TOP_HEIGHT = PropertyDefinition
            .of("dock-top-height", ConstraintConverter.INSTANCE);

    /**
     * CSS property definition for the bottom region height.
     */
    public static final PropertyDefinition<Constraint> DOCK_BOTTOM_HEIGHT = PropertyDefinition
            .of("dock-bottom-height", ConstraintConverter.INSTANCE);

    /**
     * CSS property definition for the left region width.
     */
    public static final PropertyDefinition<Constraint> DOCK_LEFT_WIDTH = PropertyDefinition
            .of("dock-left-width", ConstraintConverter.INSTANCE);

    /**
     * CSS property definition for the right region width.
     */
    public static final PropertyDefinition<Constraint> DOCK_RIGHT_WIDTH = PropertyDefinition
            .of("dock-right-width", ConstraintConverter.INSTANCE);

    static {
        PropertyRegistry.registerAll(DOCK_TOP_HEIGHT, DOCK_BOTTOM_HEIGHT, DOCK_LEFT_WIDTH,
                DOCK_RIGHT_WIDTH);
    }
     ...
     *
     * @param element
     *            the top element (e.g., a header bar)
     ...
     *            the top element (e.g., a header bar)
     * @param height
     *            the height constraint for this region
     * @return this dock for method chaining
     */
     ...
     *
     * @param element
     *            the bottom element (e.g., a status bar)
     * @return this dock for method chaining
     */
     ...
     *
     * @param element
     *            the left element (e.g., a sidebar)
     * @return this dock for method chaining
     */
     ...
     *            the left element (e.g., a sidebar)
     * @param width
     *            the width constraint for this region
     * @return this dock for method chaining
     */
     ...
     *
     * @param element
     *            the right element (e.g., a side panel)
     * @return this dock for method chaining
     */
     ...
     *
     * @param element
     *            the center element (e.g., main content)
     * @return this dock for method chaining
     */
    ...
    public int preferredWidth() {
        ...
        int leftW = leftElement != null
                ? constraintHint(resolveLeftWidth(resolver, leftElement))
                : 0;
        int centerW = centerElement != null ? centerElement.preferredWidth() : 0;
        int rightW = rightElement != null
                ? constraintHint(resolveRightWidth(resolver, rightElement))
                : 0;
    ...
    public int preferredHeight(int availableWidth, RenderContext context) {
        StylePropertyResolver resolver = StylePropertyResolver.empty();
        int topH = topElement != null
                ? constraintHint(resolveTopHeight(resolver, topElement, availableWidth, context))
                : 0;
        int bottomH = bottomElement != null
                ? constraintHint(
                        resolveBottomHeight(resolver, bottomElement, availableWidth, context))
                : 0;

        int leftH = leftElement != null ? leftElement.preferredHeight(availableWidth, context) : 0;
        int centerH = centerElement != null
                ? centerElement.preferredHeight(availableWidth, context)
                : 0;
        int rightH = rightElement != null
                ? rightElement.preferredHeight(availableWidth, context)
                : 0;
    ...
    protected void renderContent(Frame frame, Rect area, RenderContext context) {
        ...
        Constraint effectiveTopHeight = resolveTopHeight(resolver, topElement,
                effectiveArea.width(), context);
        Constraint effectiveBottomHeight = resolveBottomHeight(resolver, bottomElement,
                effectiveArea.width(), context);
        ...
        Widget topWidget = topElement != null
                ? (a, b) -> context.renderChild(topElement, frame, a)
                : null;
        Widget bottomWidget = bottomElement != null
                ? (a, b) -> context.renderChild(bottomElement, frame, a)
                : null;
        Widget leftWidget = leftElement != null
                ? (a, b) -> context.renderChild(leftElement, frame, a)
                : null;
        Widget rightWidget = rightElement != null
                ? (a, b) -> context.renderChild(rightElement, frame, a)
                : null;
        Widget centerWidget = centerElement != null
                ? (a, b) -> context.renderChild(centerElement, frame, a)
                : null;

        // Build and render the Dock widget
        Dock dock = Dock.builder().top(topWidget).bottom(bottomWidget).left(leftWidget)
                .right(rightWidget).center(centerWidget).topHeight(effectiveTopHeight)
                .bottomHeight(effectiveBottomHeight).leftWidth(effectiveLeftWidth)
                .rightWidth(effectiveRightWidth).build();

        frame.renderWidget(dock, effectiveArea);
    }

    private Constraint resolveTopHeight(StylePropertyResolver resolver, Element element,
            int availableWidth, RenderContext context) {
    ...
    private Constraint resolveBottomHeight(StylePropertyResolver resolver, Element element,
            int availableWidth, RenderContext context) {
        // Use standard resolution: programmatic → CSS → null
