 ...
 * <p>
 * TamboUI uses a dedicated render thread model similar to JavaFX. All rendering
 * operations must happen on the render thread. This class provides methods to
 * check if the current thread is the render thread and to assert that code is
 * running on the render thread.
 * <p>
 * The render thread is set when {@link TuiRunner#run} starts and cleared when
 * it exits.
 * <p>
 * <b>Usage:</b>
 * 
...
public final class RenderThread {
     ...
     * <p>
     * This should be called at the start of any method that must only be executed
     ...
     *             if a render thread has been set and this is not it
     */
    public static void checkRenderThread() {
        ...
        if (ui != null && Thread.currentThread() != ui) {
            ...
            throw new TuiException("Must be called on render thread. Current: " + current.getName()
     ...
     *
     * @param thread
     *            the thread to set as the render thread
     */
