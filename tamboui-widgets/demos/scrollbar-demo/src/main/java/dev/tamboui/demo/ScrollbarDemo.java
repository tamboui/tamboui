 ...
 * <p>
 * Demonstrates scrollbars with different orientations, styles, and integration
...
public class ScrollbarDemo {
    ...
    private ScrollbarDemo() {
        ...
        for (int i = 1; i <= ITEM_COUNT; i++) {
            items.add("Item " + i
                    + " - This is a sample list item with some extra text for horizontal scrolling");
        }
        listState.select(0);
    ...
    private void renderHeader(Frame frame, Rect area) {
        Block headerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" TamboUI ").bold().cyan(),
                        Span.raw("Scrollbar Demo ").yellow())).centered())
                .build();
    ...
    private void renderListWithScrollbar(Frame frame, Rect area) {
        Block block = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" List with Scrollbar ").green()))).build();

        Rect innerArea = block.inner(area);
        frame.renderWidget(block, area);

        // Reserve space for scrollbar (1 column on right)
        Rect listArea = new Rect(innerArea.x(), innerArea.y(), innerArea.width() - 1,
                innerArea.height());
        Rect scrollbarArea = new Rect(innerArea.right() - 1, innerArea.y(), 1, innerArea.height());

        // Create list items
        List<ListItem> listItems = items.stream().map(item -> ListItem.from(Text.from(item)))
                .toList();

        ListWidget list = ListWidget.builder().items(listItems)
                .highlightStyle(Style.EMPTY.fg(Color.YELLOW).bold()).highlightSymbol("> ").build();
        ...
        Scrollbar scrollbar = Scrollbar.builder().orientation(ScrollbarOrientation.VERTICAL_RIGHT)
                ...
                .trackStyle(Style.EMPTY.fg(Color.DARK_GRAY)).beginSymbol("▲").endSymbol("▼")
                .build();
    ...
    private void renderScrollbarShowcase(Frame frame, Rect area) {
        Block block = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" Scrollbar Styles ").magenta()))).build();

        Rect innerArea = block.inner(area);
        frame.renderWidget(block, area);

        // Split into rows for different scrollbar examples
        List<Rect> rows = Layout.vertical().constraints(Constraint.length(3), // Description
                Constraint.length(5), // Horizontal scrollbar example (needs room for block borders
                                      // + scrollbar)
        ...
        Paragraph desc = Paragraph.builder()
                .text(Text.from(Line.from(Span.raw("Use "), Span.raw("h/l").bold().yellow(),
                        Span.raw(" or "), Span.raw("←/→").bold().yellow(),
    ...
    private void renderHorizontalExample(Frame frame, Rect area) {
        ...
        Rect contentArea = new Rect(innerArea.x(), innerArea.y(), innerArea.width(),
                innerArea.height() - 1);
        Rect scrollbarArea = new Rect(innerArea.x(), innerArea.bottom() - 1, innerArea.width(), 1);
        ...
        Paragraph para = Paragraph.builder().text(Text.from(content)).build();
        ...
        Scrollbar scrollbar = Scrollbar.builder()
                ...
                .symbols(Scrollbar.SymbolSet.HORIZONTAL).thumbStyle(Style.EMPTY.fg(Color.CYAN))
    ...
    private void renderVerticalStylesShowcase(Frame frame, Rect area) {
        ...
        List<Rect> columns = Layout.horizontal().constraints(Constraint.ratio(1, 4),
                Constraint.ratio(1, 4), Constraint.ratio(1, 4), Constraint.ratio(1, 4)).split(area);

        // Style 1: Default
        renderScrollbarStyle(frame, columns.get(0), "Default",
                Scrollbar.builder().orientation(ScrollbarOrientation.VERTICAL_RIGHT).build(),
                Style.EMPTY.fg(Color.WHITE));

        // Style 2: Double line with arrows
        renderScrollbarStyle(frame, columns.get(1), "Double",
                Scrollbar.builder().orientation(ScrollbarOrientation.VERTICAL_RIGHT)
                        .symbols(Scrollbar.SymbolSet.DOUBLE_VERTICAL)
                        .thumbStyle(Style.EMPTY.fg(Color.GREEN)).build(),
                Style.EMPTY.fg(Color.GREEN));

        // Style 3: Custom symbols
        renderScrollbarStyle(frame, columns.get(2), "Custom",
                Scrollbar.builder().orientation(ScrollbarOrientation.VERTICAL_RIGHT)
                        .trackSymbol("░").thumbSymbol("▓").thumbStyle(Style.EMPTY.fg(Color.YELLOW))
                        .trackStyle(Style.EMPTY.fg(Color.DARK_GRAY)).build(),
        ...
        renderScrollbarStyle(frame, columns.get(3), "Minimal",
                Scrollbar.builder().orientation(ScrollbarOrientation.VERTICAL_RIGHT)
                        .trackSymbol("│").thumbSymbol("█").thumbStyle(Style.EMPTY.fg(Color.RED))
                        .build(),
                Style.EMPTY.fg(Color.RED));
    }

    private void renderScrollbarStyle(Frame frame, Rect area, String title, Scrollbar scrollbar,
            Style borderStyle) {
        Block block = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                .borderStyle(borderStyle).title(Title.from(Line.from(Span.raw(" " + title + " "))))
                .build();
    ...
    private void renderFooter(Frame frame, Rect area) {
        Line helpLine = Line.from(Span.raw(" ↑/k").bold().yellow(), Span.raw(" Up  ").dim(),
                Span.raw("↓/j").bold().yellow(), Span.raw(" Down  ").dim(),
                Span.raw("g").bold().yellow(), Span.raw(" First  ").dim(),
                Span.raw("G").bold().yellow(), Span.raw(" Last  ").dim(),
                Span.raw("Space").bold().yellow(), Span.raw(" Page  ").dim(),
