 ...
 * <p>
 * Demonstrates various Paragraph features: - Text alignment (left, center,
 * right) - Text wrapping (word, character) - Multiple lines with different
 * styles - Bold, italic, underlined text - Colored text - Scroll support -
 * Block integration - Hyperlinks (OSC8)
 */
public class ParagraphDemo {
    ...
    private void renderHeader(Frame frame, Rect area) {
        Block headerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" TamboUI ").bold().cyan(),
                        Span.raw("Paragraph Demo ").yellow())).centered())
                .build();
    ...
    private void renderCenteredParagraph(Frame frame, Rect area) {
        Text text = Text.from(Line.from("Centered text"), Line.from("with multiple lines."),
                Line.from(
                        Span.raw("Visit "), Span.raw("https://tamboui.dev")
                                .hyperlink("https://tamboui.dev").underlined().cyan(),
                        Span.raw(" for docs!")));

        Paragraph paragraph = Paragraph.builder().text(text).style(Style.EMPTY.fg(Color.WHITE))
                .alignment(Alignment.CENTER)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.BLUE))
                        .title(Title.from("Centered Alignment").centered()).build())
                .build();
    ...
    private void renderWrappedParagraph(Frame frame, Rect area) {
        ...
        Text text = Text.from(Line.from("Recipe: Ratatouille").bold(),
                Line.from("Ingredients:").bold(),
                Line.from(Span.raw("Bell Peppers"), Span.raw(", Eggplant").italic(),
                        Span.raw(", Tomatoes").bold(), Span.raw(", Onion")),
                Line.from(Span.raw("Secret Ingredient: ").underlined(),
                        Span.styled(secretIngredient, Style.EMPTY.fg(Color.RED).bold())),
                Line.from("Instructions:").bold().fg(Color.YELLOW),
                Line.from(longLine).fg(Color.GREEN).italic(), Line.from(""),
                Line.from(Span.raw("Full recipe at: ").dim(),
                        Span.raw("https://example.com/ratatouille")
                                .hyperlink("https://example.com/ratatouille", "recipe-link")
                                .underlined().cyan()));

        Paragraph paragraph = Paragraph.builder().text(text).style(Style.EMPTY.fg(Color.WHITE))
                .overflow(Overflow.WRAP_CHARACTER).scroll(scrollOffset)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.GREEN))
                        .title(Title.from("Wrapped Text with Styles").centered()).build())
                .build();
    ...
    private void renderFooter(Frame frame, Rect area) {
        Line helpLine = Line.from(Span.raw(" j/k").bold().yellow(), Span.raw(" Scroll  ").dim(),
