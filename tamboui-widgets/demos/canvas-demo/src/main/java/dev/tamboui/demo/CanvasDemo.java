 ...
 * <p>
 * Demonstrates drawing shapes (lines, rectangles, circles, points) with
 * different markers (braille, half-block, dot, block).
 */
public class CanvasDemo {
    ...
    private void renderHeader(Frame frame, Rect area) {
        Block headerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title
                        .from(dev.tamboui.text.Line.from(Span.raw(" TamboUI ").bold().cyan(),
    ...
    private void renderMainCanvas(Frame frame, Rect area) {
        ...
        var canvas = Canvas.builder().xBounds(0, 100).yBounds(0, 100).marker(marker)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.BLUE))
                        .title(Title
                                .from(dev.tamboui.text.Line.from(Span.raw(" Space Scene ").blue())))
                        .build())
                .paint(ctx -> {
    ...
    private void renderShapesCanvas(Frame frame, Rect area) {
        ...
        var canvas = Canvas.builder().xBounds(0, 100).yBounds(0, 100).marker(marker)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        ...
                        .title(Title.from(dev.tamboui.text.Line.from(Span.raw(" Shapes ").green())))
                        .build())
                .paint(ctx -> {
                    // Rectangle
                    ...
                    // Points
                    double[][] pointData = {{60, 70}, {65, 75}, {70, 70}, {75, 75}, {80, 70},
                            {85, 75}, {90, 70}};
    ...
    private void renderAnimatedCanvas(Frame frame, Rect area) {
        ...
        var canvas = Canvas.builder().xBounds(-50, 50).yBounds(-50, 50).marker(marker)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.MAGENTA))
                        .title(Title
                                .from(dev.tamboui.text.Line.from(Span.raw(" Rotating ").magenta())))
                        .build())
                .paint(ctx -> {
                    // Draw rotating lines
                    for (int i = 0; i < 8; i++) {
    ...
    private void renderFooter(Frame frame, Rect area) {
        dev.tamboui.text.Line helpLine = dev.tamboui.text.Line.from(Span.raw(" Frame: ").dim(),
                Span.raw(String.valueOf(frameCount)).bold().cyan(), Span.raw("   "),
                Span.raw("m").bold().yellow(), Span.raw(" Change Marker").dim(), Span.raw("   "),
