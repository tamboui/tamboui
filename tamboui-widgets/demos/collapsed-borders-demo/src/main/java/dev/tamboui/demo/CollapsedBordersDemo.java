 ...
 * <p>
 * Demonstrates: - Multiple blocks with overlapping borders - Selected pane with
 * thick border - Border style changes on selection - Interactive selection
 * (arrow keys) - Rendering order (selected last)
 * <p>
 * This demo uses Spacing.overlap() and MergeStrategy.EXACT to achieve collapsed
 * borders, where adjacent blocks share border pixels.
 */
public class CollapsedBordersDemo {
    ...
    private void ui(Frame frame) {
        ...
        var rows = Layout.vertical().constraints(Constraint.length(1), Constraint.fill(1))
                .split(area);
    ...
    private void renderTitle(Frame frame, Rect area) {
        Line title = Line.from(Span.raw("Block With Collapsed Borders").bold(),
                Span.raw(" (Press 'q' to quit)"));
        Paragraph paragraph = Paragraph.builder().text(Text.from(title))
    ...
    private void renderBlocks(Frame frame, Rect area) {
        ...
        var horizontalCols = Layout.horizontal().constraints(Constraint.fill(1), Constraint.fill(1))
                .spacing(Spacing.overlap(1)) // Overlap by 1 cell for border merging
                .split(middle);
        ...
        for (Map.Entry<Pane, Rect> entry : panes.entrySet()) {
            if (entry.getKey() != selectedPane) {
                Block block = Block.builder().borders(Borders.ALL).borderType(BorderType.PLAIN)
                        .mergeBorders(MergeStrategy.EXACT) // Enable border merging
                        .title(Title.from(getPaneTitle(entry.getKey()))).build();
        ...
        if (selectedArea != null) {
            Block block = Block.builder().borders(Borders.ALL).borderType(BorderType.THICK)
                    .borderStyle(Style.EMPTY.fg(Color.YELLOW)).mergeBorders(MergeStrategy.EXACT) // Enable
                                                                                                 // border
                                                                                                 // merging
                    .title(Title.from(getPaneTitle(selectedPane))).build();
