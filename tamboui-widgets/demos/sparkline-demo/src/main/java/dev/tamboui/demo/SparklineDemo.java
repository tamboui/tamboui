 ...
 * <p>
 * Demonstrates sparklines with different styles, bar sets, and animated data
 * updates.
 */
public class SparklineDemo {
    ...
    private void updateData() {
        ...
        memoryData[DATA_SIZE - 1] = clamp(memoryData[DATA_SIZE - 2] + random.nextInt(11) - 5, 40,
                90);
        networkData[DATA_SIZE - 1] = clamp(networkData[DATA_SIZE - 2] + random.nextInt(31) - 15, 0,
                100);
    ...
    private void renderHeader(Frame frame, Rect area) {
        Block headerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" TamboUI ").bold().cyan(),
                        Span.raw("Sparkline Demo ").yellow())).centered())
                .build();
    ...
    private void renderCpuSparkline(Frame frame, Rect area) {
        ...
        Sparkline sparkline = Sparkline.builder().data(cpuData).max(100)
                .style(Style.EMPTY.fg(Color.GREEN)).barSet(Sparkline.BarSet.NINE_LEVELS)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.GREEN))
                        .title(Title.from(Line.from(Span.raw(" " + label + " ").green()))).build())
                .build();
    ...
    private void renderMemorySparkline(Frame frame, Rect area) {
        ...
        Sparkline sparkline = Sparkline.builder().data(memoryData).max(100)
                .style(Style.EMPTY.fg(Color.YELLOW)).barSet(Sparkline.BarSet.NINE_LEVELS)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.YELLOW))
                        .title(Title.from(Line.from(Span.raw(" " + label + " ").yellow()))).build())
                .build();
    ...
    private void renderNetworkSparkline(Frame frame, Rect area) {
        ...
        Sparkline sparkline = Sparkline.builder().data(networkData).max(100)
                .style(Style.EMPTY.fg(Color.CYAN)).barSet(Sparkline.BarSet.THREE_LEVELS)
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.CYAN))
                        .title(Title.from(Line.from(Span.raw(" " + label + " ").cyan()))).build())
                .build();
    ...
    private void renderDiskSparkline(Frame frame, Rect area) {
        ...
        Sparkline sparkline = Sparkline.builder().data(diskData).max(100)
                ...
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        ...
                        .title(Title.from(Line.from(Span.raw(" " + label + " (RTL) ").magenta())))
                        .build())
                .build();
