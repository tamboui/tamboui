 ...
 * <p>
 * Demonstrates: - Todo items with status (Todo/Completed) - List widget with
 * selection and navigation - Toggle status with Enter/Right arrow - Show item
 * info below the list - Different styling for completed vs todo items -
...
public class TodoListDemo {
    ...
    private TodoListDemo() {
        ...
        todoItems.add(new TodoItem(Status.TODO,
                "Rewrite everything with your programming language of choice!",
        ...
        todoItems.add(
                new TodoItem(Status.TODO, "Walk with your dog", "Max is bored, go walk with him!"));
        todoItems.add(
                new TodoItem(Status.COMPLETED, "Pay the bills", "Pay the train subscription!!!"));
    ...
    private void ui(Frame frame) {
        ...
        var layout = Layout.vertical().constraints(Constraint.length(2), // Header
    ...
    private void renderHeader(Frame frame, Rect area) {
        Paragraph header = Paragraph.builder()
                ...
                .alignment(dev.tamboui.layout.Alignment.CENTER).build();

        frame.renderWidget(header, area);
    }

    private void renderMainContent(Frame frame, Rect area) {
        var layout = Layout.vertical().constraints(Constraint.fill(), // List area
    ...
    private void renderSelectedItemInfo(Frame frame, Rect area) {
        ...
        Block block = Block.builder().borders(Borders.TOP_ONLY).borderType(BorderType.PLAIN)
                ...
                .style(Style.EMPTY.bg(Color.indexed(235))).padding(Padding.horizontal(1)).build();

        Paragraph paragraph = Paragraph.builder().text(Text.from(info)).block(block)
                .style(Style.EMPTY.fg(Color.indexed(250))).overflow(Overflow.WRAP_WORD).build();

        frame.renderWidget(paragraph, area);
    }

    private void renderFooter(Frame frame, Rect area) {
        Line helpLine = Line.from(Span.raw("Use ").dim(), Span.raw("↓↑").bold().yellow(),
                Span.raw(" to move, ").dim(), Span.raw("←").bold().yellow(),
                Span.raw(" to unselect, ").dim(), Span.raw("→/Enter").bold().yellow(),
                Span.raw(" to change status, ").dim(), Span.raw("g/G").bold().yellow(),
