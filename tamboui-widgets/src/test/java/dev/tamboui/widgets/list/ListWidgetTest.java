...
class ListWidgetTest {
    ...
    void rendersItems() {
        List<ListItem> items = Arrays.asList(ListItem.from("Item 1"), ListItem.from("Item 2"),
                ListItem.from("Item 3"));
        ListWidget list = ListWidget.builder().items(items).highlightSymbol("") // No highlight
                                                                                // symbol for simple
                                                                                // rendering
                .build();
        ...
        assertThat(buffer).hasCellAt(0, 0, new Cell("I", Style.EMPTY))
    ...
    void withSelection() {
        ...
        ListWidget list = ListWidget.builder().items(items).highlightStyle(highlightStyle).build();
    ...
    void withHighlightSymbol() {
        List<ListItem> items = Arrays.asList(ListItem.from("Item 1"), ListItem.from("Item 2"));
        ListWidget list = ListWidget.builder().items(items).highlightSymbol("> ").build();
        Rect area = new Rect(0, 0, 20, 2);
        Buffer buffer = Buffer.empty(area);
        ListState state = new ListState();
        state.select(0);
        ...
        assertThat(buffer).hasCellAt(0, 0, new Cell(">", Style.EMPTY))
    ...
    void withBottomToTopDirection() {
        List<ListItem> items = Arrays.asList(ListItem.from("Item 1"), ListItem.from("Item 2"),
                ListItem.from("Item 3"));
        ListWidget list = ListWidget.builder().items(items).direction(ListDirection.BOTTOM_TO_TOP)
        ...
        // In bottom-to-top, first item should be at bottom (selected, so has reversed
        // style)
        Style highlightStyle = Style.EMPTY.reversed();
        assertThat(buffer).hasCellAt(0, 2, new Cell("I", highlightStyle)) // Item 1 at bottom
                                                                          // (selected)
    ...
    void withRepeatHighlightSymbol() {
        List<ListItem> items = Arrays.asList(ListItem
                .from(Text.from(Line.from("Line 1"), Line.from("Line 2"), Line.from("Line 3"))));
        ListWidget list = ListWidget.builder().items(items).highlightSymbol("> ")
                .repeatHighlightSymbol(true).build();
        ...
        // All three lines should have the highlight symbol (symbol uses default style,
        // not highlightStyle)
    ...
    void withoutRepeatHighlightSymbol() {
        ...
        dev.tamboui.assertj.BufferAssertions.assertThat(buffer).hasCellAt(0, 0,
                new Cell(">", Style.EMPTY));
        // Second and third lines should not have the symbol but should be indented
        // (content starts after symbol space)
        // Content has reversed style (default highlightStyle)
        Style highlightStyle = Style.EMPTY.reversed();
        dev.tamboui.assertj.BufferAssertions.assertThat(buffer)
                .hasCellAt(2, 1, new Cell("L", highlightStyle)) // Line 2 content (indented,
                                                                // selected)
                .hasCellAt(2, 2, new Cell("L", highlightStyle)); // Line 3 content (indented,
                                                                 // selected)
    }
    ...
    void clipOverflow() {
        List<ListItem> items = Arrays.asList(ListItem.from("Hello World This Is Long"));
        ListWidget list = ListWidget.builder().items(items).overflow(Overflow.CLIP)
    ...
    void ellipsisOverflow() {
        List<ListItem> items = Arrays.asList(ListItem.from("Hello World"));
        ListWidget list = ListWidget.builder().items(items).overflow(Overflow.ELLIPSIS)
                .highlightSymbol("").build();
    ...
    void ellipsisStartOverflow() {
        List<ListItem> items = Arrays.asList(ListItem.from("Hello World"));
        ListWidget list = ListWidget.builder().items(items).overflow(Overflow.ELLIPSIS_START)
                .highlightSymbol("").build();
        Rect area = new Rect(0, 0, 8, 1);
        Buffer buffer = Buffer.empty(area);
        ListState state = new ListState();
    ...
    void ellipsisMiddleOverflow() {
        List<ListItem> items = Arrays.asList(ListItem.from("Hello World"));
        ListWidget list = ListWidget.builder().items(items).overflow(Overflow.ELLIPSIS_MIDDLE)
                .highlightSymbol("").build();
        Rect area = new Rect(0, 0, 8, 1);
        Buffer buffer = Buffer.empty(area);
        ListState state = new ListState();
    ...
    void overflowTextFits() {
        List<ListItem> items = Arrays.asList(ListItem.from("Hi"));
        ListWidget list = ListWidget.builder().items(items).overflow(Overflow.ELLIPSIS)
                .highlightSymbol("").build();
    ...
    void overflowWithHighlightSymbol() {
        ...
        // Note: The highlight style is only applied to the content area, not the symbol
        // area
        // (see ListWidget line 237-238: "Only fill the content area, not the symbol
        // area, so symbol keeps its own style")
        Style highlightStyle = Style.EMPTY.reversed();
        Buffer expected = Buffer.empty(area);
