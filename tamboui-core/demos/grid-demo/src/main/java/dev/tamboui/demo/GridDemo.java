 ...
 * <p>
 * Demonstrates CSS Grid-inspired layout features: - Grid with explicit column
 * count - Horizontal and vertical gutter spacing - Column constraints (fixed
 * and fill) - Row constraints - Flex modes for space distribution
 */
public class GridDemo {
    ...
    private void renderHeader(Frame frame, Rect area) {
        Block headerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" TamboUI ").bold().cyan(),
                        Span.raw("Grid Demo ").yellow())).centered())
                .build();
    ...
    private void renderDashboardGrid(Frame frame, Rect area) {
        Block outerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                ...
                .title(Title.from(Line.from(Span.raw(" Dashboard Grid ").bold().green(),
        ...
        Grid grid = Grid.builder().children(metricPanel("CPU", "45%", Color.GREEN),
                metricPanel("Memory", "72%", Color.YELLOW), metricPanel("Disk", "23%", Color.CYAN),
                metricPanel("Net ↑", "1.2 MB/s", Color.BLUE),
                metricPanel("Net ↓", "5.7 MB/s", Color.MAGENTA),
                metricPanel("Uptime", "3d 14h", Color.WHITE)).columnCount(3).horizontalGutter(1)
                .verticalGutter(1).build();
    ...
    private void renderConstrainedGrid(Frame frame, Rect area) {
        Block outerBlock = Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                .borderStyle(Style.EMPTY.fg(Color.DARK_GRAY))
                .title(Title.from(Line.from(Span.raw(" Constrained Grid ").bold().blue(),
        ...
        Grid grid = Grid.builder()
                .children(labelPanel("Status", "● Online", Color.GREEN),
                ...
                .columnCount(3)
                .columnConstraints(Constraint.length(16), Constraint.fill(), Constraint.fill())
                .horizontalGutter(2).verticalGutter(1).build();
    ...
    private static Paragraph metricPanel(String label, String value, Color color) {
        return Paragraph.builder().text(Text.from(Line.from(Span.raw(value).bold().fg(color))))
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(color)).title(Title.from(label)).build())
                .build();
    }
    ...
    private static Paragraph labelPanel(String label, String value, Color color) {
        return Paragraph.builder().text(Text.from(Line.from(Span.raw(value).fg(color))))
                .block(Block.builder().borders(Borders.ALL).borderType(BorderType.ROUNDED)
                        .borderStyle(Style.EMPTY.fg(Color.DARK_GRAY))
                        .title(Title.from(Line.from(Span.raw(label).bold().fg(color)))).build())
                .build();
    }
