...
class DockTest {
    ...
    void allFiveRegions() {
        ...
        Dock.builder().top(fillingWidget("T")).bottom(fillingWidget("B")).left(fillingWidget("L"))
                .right(fillingWidget("R")).center(fillingWidget("C"))
                .topHeight(Constraint.length(2)).bottomHeight(Constraint.length(2))
                .leftWidth(Constraint.length(5)).rightWidth(Constraint.length(5)).build()
                .render(area, buffer);

        // Top region: y=0..1, full width
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "T").hasSymbolAt(15, 1, "T");

        // Bottom region: y=8..9, full width
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 8, "B").hasSymbolAt(15, 9, "B");

        // Left region: y=2..7, x=0..4
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 2, "L").hasSymbolAt(4, 5, "L");

        // Right region: y=2..7, x=25..29
        BufferAssertions.assertThat(buffer).hasSymbolAt(25, 2, "R").hasSymbolAt(29, 5, "R");

        // Center region: y=2..7, x=5..24
        BufferAssertions.assertThat(buffer).hasSymbolAt(5, 2, "C").hasSymbolAt(20, 5, "C");
    }
    ...
    void centerOnly() {
        ...
        Dock.builder().center(fillingWidget("C")).build().render(area, buffer);

        // Center should fill entire area when no other regions
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "C").hasSymbolAt(10, 5, "C")
                .hasSymbolAt(19, 9, "C");
    }
    ...
    void topAndCenter() {
        ...
        Dock.builder().top(fillingWidget("T")).center(fillingWidget("C"))
                .topHeight(Constraint.length(2)).build().render(area, buffer);

        // Top: y=0..1
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "T").hasSymbolAt(0, 1, "T");

        // Center: y=2..4
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 2, "C").hasSymbolAt(0, 4, "C");
    }
    ...
    void noHeaderOrFooter() {
        ...
        Dock.builder().left(fillingWidget("L")).center(fillingWidget("C")).right(fillingWidget("R"))
                .leftWidth(Constraint.length(5)).rightWidth(Constraint.length(5)).build()
                .render(area, buffer);

        // Left: x=0..4
        BufferAssertions.assertThat(buffer).hasSymbolAt(0, 0, "L").hasSymbolAt(4, 2, "L");

        // Center: x=5..24
        BufferAssertions.assertThat(buffer).hasSymbolAt(5, 0, "C").hasSymbolAt(20, 2, "C");

        // Right: x=25..29
        BufferAssertions.assertThat(buffer).hasSymbolAt(25, 0, "R").hasSymbolAt(29, 2, "R");
    }
    ...
    void emptyArea() {
        ...
        Dock.builder().center(charWidget("A")).build().render(emptyArea, buffer);
    ...
    void noRegions() {
        ...
        Dock.builder().build().render(area, buffer);
    ...
    void customConstraints() {
        ...
        Dock.builder().top(charWidget("T")).center(charWidget("C")).topHeight(Constraint.length(3))
