...
public final class ConstraintConverter implements PropertyConverter<Constraint> {
    ...
    /**
     * Singleton instance of the constraint converter.
     */
    ...
    @Override
    public Optional<Constraint> convert(String value) {
        ...
        // Try "fit" keyword
        if ("fit".equals(normalized)) {
            return Optional.of(Constraint.fit());
        }

        // Try fill pattern: "fill" or "fill(2)"
        Matcher fillMatcher = FILL_PATTERN.matcher(normalized);
        ...
        Matcher frMatcher = FR_PATTERN.matcher(normalized);
        ...
        Matcher percentMatcher = PERCENT_PATTERN.matcher(normalized);
        ...
        Matcher ratioMatcher = RATIO_PATTERN.matcher(normalized);
        ...
        if (LENGTH_PATTERN.matcher(normalized).matches()) {
            int length = Integer.parseInt(normalized);
