 ...
 * <p>
 * Mode 2027 is a terminal feature that coordinates grapheme cluster handling
 * between applications and terminals. When enabled, the terminal signals that
 * it can handle complex grapheme clusters (ZWJ emoji sequences, regional
 * indicators) as single display units.
 * <p>
 * <b>Escape sequences:</b>
 * <ul>
 * <li>Query (DECRQM): {@code CSI ? 2027 $ p}</li>
 * <li>Response (DECRPM): {@code CSI ? 2027 ; Ps $ y} where Ps=1 enabled, Ps=2
 * disabled but recognized</li>
 ...
 *
 * @see <a href=
 *      "https://mitchellh.com/writing/grapheme-clusters-in-terminals">Grapheme
 *      Clusters in Terminals</a>
 */
public final class Mode2027Support {
     ...
     * <p>
     * Sends the DECRQM escape sequence and waits for the DECRPM response. If the
     * terminal doesn't respond within the timeout, it is considered as not
     * supporting Mode 2027.
     *
     * @param backend
    ...
    private static Mode2027Status parseResponse(Backend backend, int timeoutMs) throws IOException {
        ...
        while (System.nanoTime() < deadlineNanos) {
            int remainingTime = (int) TimeUnit.NANOSECONDS
     ...
     *
     * @param psValue
     *            the Ps value from the response
