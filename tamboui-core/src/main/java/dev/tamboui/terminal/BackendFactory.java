...
/**
 * Factory for creating {@link Backend} instances using the
 * {@link java.util.ServiceLoader} mechanism.
 * 
 * This uses the {@link SafeServiceLoader} to load the providers and skip
 * providers that fails to load.
 * <p>
 * This factory discovers {@link BackendProvider} implementations on the
 * classpath and uses them to create backend instances. When multiple providers
 * are available, they are tried in order until one successfully creates a
 * backend.
 * <p>
...
public final class BackendFactory {
     ...
     * <p>
     * Call this method early in your application (before any System.out usage) to
     * ensure all console output is captured when recording is enabled. This is
     * especially important for inline demos that print to System.out before
     * creating a Backend.
     * <p>
     * If recording is not enabled, this method does nothing. If recording is
     * already initialized, this method does nothing.
     */
     ...
     * <p>
     * This method discovers {@link BackendProvider} implementations on the
     * classpath and selects one based on the following priority:
     * <ol>
     ...
     * <ul>
     * <li>Simple name (e.g., "jline", "panama") - matches the provider's
     * {@link BackendProvider#name()}</li>
     * <li>Fully qualified class name (e.g.,
     * "dev.tamboui.backend.jline.JLineBackendProvider")</li>
     * <li>Comma-separated list (e.g., "panama,jline") - tries each in order until
     * one succeeds</li>
     * </ul>
     * <p>
     * Providers are tried in order until one successfully creates a backend. If a
     * provider fails (throws an exception), the next provider is attempted. This
     * applies both to explicitly specified providers and auto-discovered ones.
     *
     * @return a new backend instance
     * @throws IOException
     *             if backend creation fails
     * @throws IllegalStateException
     *             if no provider is found or all providers fail
     */
    public static Backend create() throws IOException {
        ...
        List<BackendProvider> providers = (userSelectedProvider != null
                && !userSelectedProvider.isEmpty())
    ...
    /**
     * Resolves providers from a user specification, returning them in the specified
     * order.
     *
     * @param providerSpec
     *            the provider specification (may be comma-separated)
     * @param allProviders
     *            all available providers from ServiceLoader
     * @return list of matching providers in the specified order
     * @throws BackendException
     *             if a backend provider is not found
     */
    private static List<BackendProvider> resolveProviders(String providerSpec,
        ...
        for (String spec : providerSpec.split(",")) {
            ...
            BackendProvider provider = allProviders.stream()
                    .filter(p -> p.name().equals(trimmedSpec)).findFirst()
    ...
    private static Backend tryProviders(List<BackendProvider> providers) {
        if (providers.isEmpty()) {
            throw new BackendException("No BackendProvider found on classpath.\n"
                    + "Add a backend dependency such as tamboui-jline3-backend or tamboui-panama-backend.");
        }
        ...
        throw new BackendException("All backend providers failed to create a backend.\n" + "Tried: "
                + formatAvailableProviders(providers) + "\n" + "Errors:\n" + errors);
    }

    /**
     * Formats a list of providers into a user-friendly string showing both names
     * and class names.
     *
     * @param providers
    ...
    private static String formatAvailableProviders(List<BackendProvider> providers) {
        return providers.stream().map(p -> p.name() + " (" + p.getClass().getName() + ")")
