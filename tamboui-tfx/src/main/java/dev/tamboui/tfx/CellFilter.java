...
/**
 * A filter that enables effects to operate on specific cells based on various
 * criteria.
 * <p>
 * CellFilter provides a flexible, composable way to select cells for applying
 * effects. Filters can match cells based on their properties (colors, position,
 * content) or custom predicates, and can be combined using logical operations
 * (AND, OR, NOR).
 * <p>
 * <b>Design Philosophy:</b>
 * <ul>
 * <li><b>Composability:</b> Simple filters can be combined to create complex
 * selection patterns using {@code allOf}, {@code anyOf}, and
 * {@code noneOf}.</li>
 * <li><b>Performance:</b> Filters are evaluated during cell iteration, allowing
 * effects to skip irrelevant cells efficiently.</li>
 * <li><b>Flexibility:</b> Supports both spatial (area, margin) and
 * content-based (color, text) filtering.</li>
 * </ul>
 * <p>
 * <b>Filter Types:</b>
 * <ul>
 ...
 * <pre>{@code
 * // Simple filter
 * Effect effect = Fx.dissolve(2000, Interpolation.Linear).withFilter(CellFilter.text());
 * 
 * // Combined filters
 * CellFilter combined = CellFilter.allOf(CellFilter.text(), CellFilter.fgColor(Color.WHITE),
 *         CellFilter.outer(Margin.uniform(1)));
 * Effect filtered = Fx.fadeToFg(Color.CYAN, 2000, Interpolation.SineInOut).withFilter(combined);
 * }</pre>
 * <p>
 * Filters are evaluated during effect execution, allowing effects to
 * efficiently skip cells that don't match the filter criteria.
 */
public abstract class CellFilter {
     ...
     *
     * @param margin
     *            the margin defining the inner region
     ...
     *
     * @param margin
     *            the margin defining the outer region
     ...
     *
     * @param predicate
     *            the position predicate to evaluate
     ...
     *
     * @param predicate
     *            the cell predicate to evaluate
    ...
    private static final class Text extends CellFilter {
        ...
        private static boolean matchesCell(Cell cell) {
            ...
            // Check if symbol contains text characters (letters, numbers, punctuation, and
            // common symbols)
            // Base matches Rust: is_alphabetic() || is_numeric() || "
            // ?!.,:;()".contains(ch)
            // Extended to include common symbols that are part of text content:
            // @#$%^&*+-=_[ ]{}|\/<>"'~`
