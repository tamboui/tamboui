 ...
 * the animation spreads outward from a focal point. The transition width
 * controls how smoothly the effect transitions from inactive to active regions.
 * <p>
 * <b>Key Concepts:</b>
 * <ul>
...
public final class RadialPattern implements Pattern {
    ...
    /**
     * Creates a radial pattern with custom center point (0.0-1.0 normalized
     * coordinates).
     *
     * @param centerX
     ...
     */
    public static RadialPattern withTransition(float centerX, float centerY,
     ...
     *
     * @param width
     *            the new transition width in terminal cells
     ...
     *
     * @param centerX
     *            the new center X position (0.0-1.0)
     * @param centerY
     *            the new center Y position (0.0-1.0)
    ...
    public float mapAlpha(float globalAlpha, Position position, Rect area) {
        ...
        float maxDistance = dev.tamboui.tfx.TFxMath.sqrt(
                (area.width() * area.width()) + (2.0f * area.height() * 2.0f * area.height()));
