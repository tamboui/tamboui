...
public final class TransitionProgress {
    ...
    /**
     * Creates a new transition progress handler with the specified transition
     * width.
     * 
     * @param width
     *            Width of the gradient transition zone in terminal cells (minimum
     *            0.1)
     */
     ...
     * 
     * @param globalAlpha
     *            Global animation progress (0.0-1.0)
     * @param position
     *            Position along the pattern dimension
     * @param maxRange
     *            Maximum value for the position dimension
    ...
    /**
     * Maps radial patterns where smaller distances (closer to center) should be
     * more active.
     * 
     * @param globalAlpha
     *            Global animation progress (0.0-1.0)
    ...
    public float mapRadial(float globalAlpha, float distance, float maxRange) {
        ...
        // For radial: we want 1.0 when distance <= threshold, 0.0 when distance >
        ...
        if (distance <= threshold) {
            ...
            return 1.0f - java.lang.Math.max(0.0f,
                    java.lang.Math.min(1.0f, distanceIntoTransition / transitionWidth));
        } else {
            return 0.0f;
        }
    }
    ...
    /**
     * Maps discrete threshold patterns where cells have distinct activation
     * thresholds.
     * <p>
     ...
     *            Global animation progress (0.0-1.0)
     * @param cellThreshold
     *            The discrete threshold for this cell (e.g., 0.0 for white, 0.5 for
     *            black)
     * @return The mapped alpha value (0.0-1.0)
     */
    public float mapThreshold(float globalAlpha, float cellThreshold) {
        ...
        if (scaledAlpha >= cellThreshold + transitionWidth) {
            ...
            return java.lang.Math.max(0.0f,
                    java.lang.Math.min(1.0f, distanceIntoTransition / transitionWidth));
        } else {
            return 0.0f;
        }
