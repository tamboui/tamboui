 ...
 * in one of the four cardinal directions (left-to-right, right-to-left,
 * up-to-down, down-to-up). The effect creates a "wave" or "wipe" animation that
 * moves across the screen.
 * <p>
 * <b>Design Philosophy:</b>
 * <p>
 * Sweep patterns are ideal for directional reveals, transitions, and wipes.
 * They provide a simple, predictable way to control effect progression based on
 * position along one axis.
 * <p>
 * <b>Key Concepts:</b>
 * <ul>
 * <li><b>Direction:</b> The cardinal direction of the sweep (horizontal or
 * vertical)</li>
 * <li><b>Transition Width:</b> Controls the gradient zone where the effect
 * transitions from inactive to active (in terminal cells). Larger values create
 * smoother, more gradual transitions.</li>
 ...
 * }</pre>
 * <p>
 * The pattern uses {@link TransitionProgress#mapSpatial(float, float, float)}
 * internally to calculate position-specific alpha values based on the cell's
 * position along the sweep axis and the global animation progress. This
 * provides smooth gradient transitions controlled by the transition width
 * parameter.
 * <p>
 * <b>Note:</b> {@link dev.tamboui.tfx.SlidingWindowAlpha} is used by
 * {@link dev.tamboui.tfx.effects.SweepShader} for sweep effects, but
 * {@code SweepPattern} uses {@code TransitionProgress} for pattern-based
 * spatial mapping when applied to other effects (like fade effects).
 */
public final class SweepPattern implements Pattern {
     ...
     *            width of the gradient transition zone in terminal cells
