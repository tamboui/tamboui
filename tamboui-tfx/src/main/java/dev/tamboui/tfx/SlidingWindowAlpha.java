 ...
 * {@link dev.tamboui.tfx.effects.SweepShader}) to calculate position-specific
 * alpha values based on a "sliding window" that moves across the terminal area.
 * <p>
 * <b>Design Philosophy:</b>
 * <p>
 * The sliding window creates a gradient zone that moves across the area as the
 * animation progresses. Cells before the window are fully inactive (alpha =
 * 0.0), cells within the window have a gradient from 0.0 to 1.0, and cells
 * after the window are fully active (alpha = 1.0). This creates smooth
 * directional transitions.
 * <p>
 * <b>Key Concepts:</b>
 * <ul>
 * <li><b>Sliding Window:</b> A gradient zone that moves across the area based
 * on animation progress</li>
 * <li><b>Gradient Length:</b> The width of the transition zone in terminal
 * cells</li>
 * <li><b>Progress:</b> The global animation progress (0.0 to 1.0) that
 * determines window position</li>
 * </ul>
 * <p>
 * <b>Usage:</b>
 * <p>
 * This class is primarily used internally by
 * {@link dev.tamboui.tfx.effects.SweepShader} to implement sweep effects. It's
 * not typically used directly by application code.
 * <p>
 * <b>Note:</b> For pattern-based spatial effects (like
 * {@link dev.tamboui.tfx.pattern.SweepPattern}), use
 * {@link dev.tamboui.tfx.pattern.TransitionProgress} instead, which provides a
 * different approach to spatial mapping that works better with the pattern
 * system.
 */
public final class SlidingWindowAlpha {
     ...
     *
     * @param direction
     *            the direction of the sliding window
     * @param area
     *            the rectangular area to apply the effect to
     * @param progress
     *            the global animation progress (0.0 to 1.0)
     * @param gradientLength
     *            the width of the gradient transition zone
     * @return a new sliding window alpha calculator
     */
    public static SlidingWindowAlpha create(Motion direction, Rect area, float progress,
     ...
     *
     * @param position
     *            the position to calculate alpha for
    ...
    private static Gradient calculateGradient(float progress, int coordinate, int areaLen,
            float gradientLen) {
