...
/**
 * A shader that applies bidirectional expansion effects using two opposing
 * stretch shaders.
 * <p>
 * Creates expansion animations that grow outward from the center in both
 * directions simultaneously, either horizontally or vertically.
 */
public final class ExpandShader implements Shader {
     ...
     *
     * @param direction
     *            the expansion direction (horizontal or vertical)
     * @param style
     *            the style to apply during expansion
     * @param timer
     *            the effect timer controlling the expansion duration
     */
    ...
    public TFxDuration process(TFxDuration duration, Buffer buffer, Rect area) {
        ...
        if (direction == ExpandDirection.HORIZONTAL) {
            ...
            areaB = new Rect(areaA.right(), effectArea.top(), effectArea.width() - halfWidth,
                    effectArea.height());
        } else { // VERTICAL
            int halfHeight = effectArea.height() / 2;
            areaA = new Rect(effectArea.left(), effectArea.top(), effectArea.width(), halfHeight);
            areaB = new Rect(effectArea.left(), areaA.bottom(), effectArea.width(),
                    effectArea.height() - halfHeight);
        }
