 ...
 * Cells gradually fill with block characters (█, ▇, ▆, etc.) based on the
 * direction.
 */
public final class SlideShader implements Shader {
     ...
     *            the length of the gradient transition zone
     * @param randomness
     *            the amount of randomness to apply to the slide
     * @param colorBehindCell
     *            the color revealed behind cells as they slide away
     * @param timer
     *            the effect timer controlling the animation
    ...
    private SlideShader(Motion direction, int gradientLength, int randomness, Color colorBehindCell,
            EffectTimer timer) {
    ...
    public void execute(TFxDuration duration, Rect area, Buffer buffer) {
        ...
        SlidingWindowAlpha windowAlpha = SlidingWindowAlpha.create(direction, effectArea, alpha,
                gradientLength + randomness);

        // Create directional variance for randomness
        DirectionalVariance axisJitter = DirectionalVariance.withRng(new SimpleRng(rng.state()),
                direction, randomness);

        CellFilter filter = cellFilter != null ? cellFilter : CellFilter.all();

        // Apply randomness based on direction
        if (randomness == 0 || direction == Motion.LEFT_TO_RIGHT
                || direction == Motion.RIGHT_TO_LEFT) {
            ...
            for (int y = effectArea.top(); y < effectArea.bottom(); y++) {
                for (int x = effectArea.left(); x < effectArea.right(); x++) {
                    ...
                    Position offsetPos = new Position(Math.max(0, pos.x()),
    ...
    private void updateCell(Buffer buffer, Position pos, Cell cell, float cellAlpha) {
        ...
        // For block characters: fg = original bg, bg = color_behind_cell (matching
        // Rust)
        dev.tamboui.style.Style blockStyle = dev.tamboui.style.Style.EMPTY.fg(blockFg)
                .bg(colorBehindCell);
    ...
    public Shader copy() {
        ...
        SlideShader copy = new SlideShader(direction, gradientLength, randomness, colorBehindCell,
                timerCopy);
        copy.area = area;
