...
public final class SweepShader implements Shader {
     ...
     *
     * @param direction
     *            the direction of the sweep animation
     * @param gradientLength
     *            the length of the gradient transition zone
     * @param randomness
     *            the amount of randomness to apply to the sweep
     * @param fadedColor
     *            the color to sweep in from
     * @param timer
    ...
    private SweepShader(Motion direction, int gradientLength, int randomness, Color fadedColor,
            EffectTimer timer) {
    ...
    private void applyAlpha(Buffer buffer, Position pos, Cell cell, float cellAlpha) {
        ...
        if (cellAlpha <= 0.0f) {
            ...
            buffer.set(pos,
                    cell.style(dev.tamboui.style.Style.EMPTY.fg(fadedColor).bg(fadedColor)));
            ...
            buffer.set(pos, cell.patchStyle(
                    dev.tamboui.style.Style.EMPTY.fg(interpolatedFg).bg(interpolatedBg)));
        }
    }
    ...
    public Shader copy() {
        ...
        SweepShader copy = new SweepShader(direction, gradientLength, randomness, fadedColor,
                timerCopy);
        copy.area = area;
