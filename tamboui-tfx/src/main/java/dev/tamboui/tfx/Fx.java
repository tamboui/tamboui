 ...
 * <p>
 * The Fx class serves as the primary entry point for creating effects. It
 * provides convenient factory methods that encapsulate the creation of shaders
 * and their wrapping in Effect instances.
 * <p>
 * <b>Design Philosophy:</b>
 * <ul>
 * <li><b>Convenience:</b> Simplifies effect creation with sensible
 * defaults</li>
 * <li><b>Consistency:</b> Provides a uniform API for all effect types</li>
 * <li><b>Discoverability:</b> All effects are accessible through a single
 * class</li>
 * </ul>
 * <p>
 ...
 * <li><b>Text Effects:</b> {@code dissolve}, {@code dissolveTo},
 * {@code coalesce}, {@code coalesceFrom}</li>
 * <li><b>Motion Effects:</b> {@code sweepIn}, {@code sweepOut},
 * {@code slideIn}, {@code slideOut}</li>
 ...
 * }</pre>
 * <p>
 * All factory methods return {@link Effect} instances that can be further
 * configured using methods like {@link Effect#withFilter(CellFilter)},
 * {@link Effect#withColorSpace(TFxColorSpace)}, and
 * {@link Effect#withPattern(dev.tamboui.tfx.pattern.Pattern)}.
 */
public final class Fx {
     ...
     * @return A fade effect
     */
    public static Effect fadeTo(Color fromColor, Color toColor, long durationMs,
     ...
     * @return A fade effect
     */
    public static Effect fadeFrom(Color fromColor, Color toColor, long durationMs,
            Interpolation interpolation) {
        return fadeFrom(fromColor, toColor, EffectTimer.fromMs(durationMs, interpolation));
    }

    /**
     * Creates a fade effect that transitions the foreground color from the
     * specified color.
     * 
     * @param fromColor
     *            The starting foreground color
     * @param timer
     *            The effect timer
     * @return A fade effect
     */
     ...
     * @return A dissolve effect
     */
    public static Effect dissolveTo(dev.tamboui.style.Style style, long durationMs,
            Interpolation interpolation) {
        return dissolveTo(style, EffectTimer.fromMs(durationMs, interpolation));
    }

    /**
     * Creates a coalesce effect (reverse dissolve) that coalesces text from random
     * positions.
     * 
     * @param timer
     *            The effect timer
    ...
    public static Effect coalesceFrom(dev.tamboui.style.Style style, EffectTimer timer) {
        return Effect
                .of(dev.tamboui.tfx.effects.DissolveShader.dissolveTo(style, timer.mirrored()));
    }
     ...
     * @return A coalesce effect
     */
    public static Effect coalesceFrom(dev.tamboui.style.Style style, long durationMs,
            Interpolation interpolation) {
     ...
     * @param gradientLength
     *            The length of the gradient transition
     * @param randomness
     *            The maximum random offset (0 for uniform)
     * @param fadedColor
     *            The color to sweep from
     * @param timer
     *            The effect timer
    ...
    public static Effect sweepIn(Motion direction, int gradientLength, int randomness,
        ...
        return Effect.of(dev.tamboui.tfx.effects.SweepShader.sweepIn(direction, gradientLength,
                randomness, fadedColor, timer));
    }
    ...
    public static Effect sweepOut(Motion direction, int gradientLength, int randomness,
        ...
        return sweepIn(direction.flipped(), gradientLength, randomness, fadedColor,
                timer.reversed());
    }
    ...
    /**
     * Creates a slide effect that slides in using block characters (shutter
     * animation).
     * 
     * @param direction
     ...
     * @param randomness
     *            The maximum random offset (0 for uniform)
     * @param colorBehindCell
     *            The color behind the sliding cells
     * @param timer
     *            The effect timer
    ...
    public static Effect slideIn(Motion direction, int gradientLength, int randomness,
        ...
        return slideOut(direction.flipped(), gradientLength, randomness, colorBehindCell, timer)
                .reversed();
    }
    ...
    /**
     * Creates a slide effect that slides out using block characters (shutter
     * animation).
     * 
     * @param direction
     *            The direction of the slide
    ...
    public static Effect slideOut(Motion direction, int gradientLength, int randomness,
        ...
        return Effect.of(dev.tamboui.tfx.effects.SlideShader.slideOut(direction, gradientLength,
                randomness, colorBehindCell, finalTimer));
    }
     ...
     * 
     * @param fg
     *            The foreground color to apply (null to skip)
     * @param bg
     *            The background color to apply (null to skip)
     * @param timer
     *            The effect timer
    ...
    /**
     * Runs the effects in sequence, one after the other. Reports completion once
     * the last effect has completed.
     * 
     * @param effects
    ...
    /**
     * Runs the effects in parallel, all at the same time. Reports completion once
     * all effects have completed.
     * 
     * @param effects
     ...
     * 
     * @param effects
     *            The list of effects to run in parallel
     * @return A parallel effect
     */
    ...
    /**
     * Creates an expand effect that grows outward from the center in both
     * directions.
     * <p>
     * The expand effect fills the area with the specified style (including
     * background color), then reveals the content as it expands. It uses block
     * characters for smooth transitions.
     * 
     * @param direction
     *            The direction of expansion (horizontal or vertical)
     * @param style
     *            The style to apply during expansion (foreground and background
     *            colors)
     * @param timer
     *            The effect timer
     * @return An expand effect
     */
    public static Effect expand(ExpandDirection direction, dev.tamboui.style.Style style,
            EffectTimer timer) {
     ...
     */
    public static Effect expand(ExpandDirection direction, dev.tamboui.style.Style style,
            long durationMs, Interpolation interpolation) {
