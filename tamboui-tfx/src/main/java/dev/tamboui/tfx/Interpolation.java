 ...
 * <p>
 * Interpolation transforms linear progress (0.0 to 1.0) into smooth animation
 * curves. Different interpolation types produce different visual effects, from
 * linear motion to bouncy, elastic, or overshooting animations.
 * <p>
 * <b>Design Philosophy:</b>
 * <p>
 * Interpolation functions separate timing concerns from rendering logic.
 * Effects use the interpolated alpha value to drive their visual
 * transformations, allowing the same effect to produce different feels with
 * different interpolation types.
 * <p>
 * <b>Interpolation Categories:</b>
 * <ul>
 * <li><b>Linear:</b> Constant speed (no easing)</li>
 * <li><b>Polynomial:</b> Quad, Cubic, Quart, Quint - progressively stronger
 * acceleration</li>
 ...
 * 
 * // Elastic effect
 * Effect elastic = Fx.sweepIn(Motion.LEFT_TO_RIGHT, 10, 0, Color.BLUE, 2000,
 *         Interpolation.ElasticOut);
 * }</pre>
 * <p>
 * <b>Choosing an Interpolation:</b>
 * <ul>
 * <li><b>SineInOut:</b> Most versatile, smooth and natural (recommended
 * default)</li>
...
public enum Interpolation {
     ...
     *
     * @param a
     *            the input alpha value (0.0 to 1.0)
    ...
    private static float easingBackInOut(float t) {
        ...
        if (t < 0.5f) {
            ...
            return (TFxMath.powi(2.0f * t - 2.0f, 2) * ((c2 + 1.0f) * (t * 2.0f - 2.0f) + c2)
                    + 2.0f) / 2.0f;
        }
    }
    ...
    private static float easingElasticIn(float t) {
        if (t == 0.0f) {
            ...
            return -TFxMath.powf(2.0f, 10.0f * (t - 1.0f))
                    * TFxMath.sin((t - 1.0f) * c4 - TFxMath.pi() / 2.0f);
        }
    }
    ...
    private static float easingElasticOut(float t) {
        if (t == 0.0f) {
            ...
            float c4 = TFxMath.tau() / 3.0f;
            return TFxMath.powf(2.0f, -10.0f * t) * TFxMath.sin(t * c4 - TFxMath.pi() / 2.0f)
                    + 1.0f;
        }
    }
