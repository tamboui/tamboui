 ...
 * <p>
 * Color spaces determine how colors are interpolated between start and end
 * values. Different color spaces produce different visual results, particularly
 * when transitioning between colors with different hues or saturation levels.
 * <p>
 * <b>Design Philosophy:</b>
 * <p>
 * Color space selection balances performance and perceptual quality. RGB is
 * fastest but can produce perceptually non-uniform transitions (e.g., muddy
 * gray midpoints). HSL and HSV provide better perceptual uniformity at the cost
 ...
 * <li><b>HSL:</b> Interpolation in Hue-Saturation-Lightness space. Default
 * choice providing good balance of performance and perceptual quality. Produces
 * smooth hue transitions and maintains saturation better than RGB.</li>
 * <li><b>HSV:</b> Interpolation in Hue-Saturation-Value space. Similar to HSL
 * but with a different perceptual model. Value represents brightness
 * differently than Lightness in HSL.</li>
 * </ul>
 * <p>
 ...
 * <li><b>HSL:</b> Use as default for most fade effects, especially when
 * transitioning between colors with different hues</li>
 * <li><b>HSV:</b> Use when you need a different perceptual model than HSL, or
 * when transitioning between colors where Value is more important than
 * Lightness</li>
 * </ul>
 * <p>
 * <b>Usage Pattern:</b>
 ...
 * }</pre>
 * <p>
 * This enum also provides utility methods for creating colors from HSL and HSV
 * values, which can be useful for creating color palettes or converting between
...
public enum TFxColorSpace {
    ...
    /**
     * Internal helper: Converts a color to HSL components [h, s, l] where: h is in
    ...
    /**
     * Internal helper: Converts a color to HSV components [h, s, v] where: h is in
     * degrees (0-360) s and v are percentages (0-100)
     */
    ...
    int[] hslToRgb(float h, float s, float l) {
        ...
        return new int[]{java.lang.Math.round((r + m) * 255.0f),
                java.lang.Math.round((g + m) * 255.0f), java.lang.Math.round((b + m) * 255.0f)};
    }
    ...
    private int[] indexedToRgb(int index) {
        if (index < 16) {
            ...
            int g = (cubeIndex / 6) % 6;
            int b = cubeIndex % 6;
            return new int[]{r == 0 ? 0 : 55 + r * 40, g == 0 ? 0 : 55 + g * 40,
                    b == 0 ? 0 : 55 + b * 40};
        } else {
     ...
     * @return An array [hue, saturation, lightness] where: - hue is in degrees
     *         (0-360) - saturation and lightness are percentages (0-100)
     */
     ...
     * @param color
     *            The color to convert
     * @return An array [hue, saturation, value] where: - hue is in degrees (0-360)
