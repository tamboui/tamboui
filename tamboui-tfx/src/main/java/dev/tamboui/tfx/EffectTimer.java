 ...
 * <li><b>Progress Calculation:</b> Computing alpha values (0.0 to 1.0) based on
 * elapsed time</li>
 * <li><b>Easing Application:</b> Applying interpolation functions for smooth
 * animations</li>
 ...
 * <p>
 * EffectTimer separates timing concerns from rendering logic. Effects use the
 * timer's {@link #alpha()} method to get a normalized progress value (0.0 to
 * 1.0) that has been transformed by the interpolation function. This allows
 * effects to focus on "what to render" rather than "when to render it."
 * <p>
 * <b>Key Concepts:</b>
 * <ul>
 ...
 * <p>
 * Use {@link #reversed()} to play an effect backwards, or {@link #mirrored()}
 * to reverse while preserving the visual curve shape (useful for ping-pong
 * effects).
 */
public final class EffectTimer {
    ...
    /**
     * Creates a new EffectTimer with the specified duration and Linear
     * interpolation.
     *
     * @param duration
     *            the total duration
     ...
     * interpolation type, mirrored() flips the interpolation to compensate for the
     * reversed direction.
     *
    ...
    /**
     * Processes the timer by reducing the remaining duration by the specified
     * amount.
     * <p>
     * For looping timers, this method handles resetting or reversing the timer when
     * it completes:
     * <ul>
     ...
     * @return The overflow duration if the timer has completed (ONCE mode only), or
     ...
     * this method always returns false since looping timers never complete on their
     * own.
     *
    ...
    public boolean equals(Object o) {
        ...
        return reverse == that.reverse && Objects.equals(remaining, that.remaining)
                && Objects.equals(total, that.total) && interpolation == that.interpolation
    ...
    public String toString() {
        return "EffectTimer{remaining=" + remaining + ", total=" + total + ", interpolation="
                + interpolation + ", reverse=" + reverse + ", loopMode=" + loopMode + "}";
    }
}
