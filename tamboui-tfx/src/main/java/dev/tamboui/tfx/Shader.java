...
/**
 * Interface representing a shader that applies visual effects to terminal
 * cells.
 * <p>
 * The Shader interface is the core abstraction for all visual effects. It
 * defines the contract for objects that can transform terminal cell appearance
 * over time.
 * <p>
 * <b>Design Philosophy:</b>
 * <ul>
 * <li><b>Separation of Timing and Rendering:</b> The default {@link #process}
 * method handles timer updates, while {@link #execute} handles the actual
 * rendering logic.</li>
 * <li><b>Flexible Configuration:</b> Shaders can be configured with filters,
 * patterns, color spaces, and areas through optional methods.</li>
 * <li><b>Composability:</b> Shaders can be copied and combined to create
 * complex effects.</li>
 * </ul>
 * <p>
 ...
 * <li>Implement {@link #execute} to perform the actual cell
 * transformations</li>
 * <li>Implement {@link #done()} to check if the effect is complete</li>
 * <li>Implement {@link #copy()} to support effect composition</li>
 * </ol>
 * <p>
 * The default {@link #process} implementation handles timer updates
 * automatically, so you typically only need to override {@link #execute}. Only
 * override {@link #process} if you need custom timer handling (e.g., for
 * effects that don't use timers).
 * <p>
...
public interface Shader {
     ...
     * <p>
     * The default implementation: 1. Updates the timer with the given duration (if
     * a timer exists) 2. Calls {@link #execute} with the current state 3. Returns
     * any overflow duration
     * <p>
     * Most effects should use this default implementation and implement
     * {@link #execute} instead. Only override this if you need custom timer
     * handling.
     * 
     * @param duration
     *            The duration to process the shader for
     * @param buffer
     *            The buffer where the shader will be applied
     * @param area
     *            The rectangular area within the buffer where the shader will be
     *            applied
     ...
     *
     * @param area
     *            the rectangular area for the effect
     */
     ...
     * <p>
     * This method reverses the timer direction, causing the effect to play
     * backwards. The default implementation reverses the timer if one exists.
     */
     ...
     * <p>
     * This method sets the loop mode on the underlying timer, controlling what
     * happens when the effect reaches its end. See {@link LoopMode} for available
     * modes.
     *
     * @param mode
