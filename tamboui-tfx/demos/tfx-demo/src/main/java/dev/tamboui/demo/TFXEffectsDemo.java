 ...
 * <p>
 * Controls: - Enter: Next transition - Backspace: Previous transition - Space:
 * Restart transition - r: Random transition - ESC: Quit
 */
public class TFXEffectsDemo {
    ...
    private void run() throws Exception {
        ...
        try (TuiRunner tui = TuiRunner.create(config)) {
            tui.run((event, runner) -> {
                ...
                Rect contentArea = new Rect(area.left() + contentX, area.top() + contentY,
    ...
    private void render(Frame frame) {
        ...
        if (fpsX >= area.left()) {
            frame.buffer().setString(fpsX, area.top(), fpsText,
                    Style.EMPTY.fg(LIGHT4).bg(DARK0_HARD));
        }
        ...
        Block block = Block.builder().borders(Borders.NONE).style(Style.EMPTY.bg(DARK0_SOFT))
                .build();
        ...
        // Create layout
        Layout layout = Layout.vertical().constraints(Constraint.length(2), // Active animation
                                                                            // label
        ...
        Text activeAnimation = Text.from(Line.from(Span.raw("Active animation: ").fg(ORANGE),
                Span.raw(effectName).fg(ORANGE_BRIGHT)));
        frame.renderWidget(
                Paragraph.builder().text(activeAnimation).style(Style.EMPTY.bg(DARK0_SOFT)).build(),
                splits.get(0));
        ...
        Text mainText = Text.from(
                ...
                Line.from(
                        "Effects such as `never_complete`, `temporary` influence or override this."),
                Line.from("Symbols: !@#$%^&*()"),
                Line.from("The text in this window will undergo a random transition"),
                Line.from("when any of the following keys are pressed:")).fg(LIGHT3);
        frame.renderWidget(
                Paragraph.builder().text(mainText).style(Style.EMPTY.bg(DARK0_SOFT)).build(),
                splits.get(1));

        // Shortcuts
        Text shortcuts = Text.from(shortcut("↵   ", "next transition"),
                shortcut("⌫   ", "previous transition"), shortcut("␣   ", "restart transition"),
                shortcut("r   ", "random transition"), shortcut("ESC ", "quit"));
        frame.renderWidget(
                Paragraph.builder().text(shortcuts).style(Style.EMPTY.bg(DARK0_SOFT)).build(),
                splits.get(2));
    ...
    private Line shortcut(String key, String desc) {
        return Line.from(Span.raw(key).fg(YELLOW_BRIGHT).bold(), Span.raw(desc).fg(LIGHT4));
    }
    ...
    private static class EffectsRepository {
        ...
        EffectsRepository() {
            ...
            // Sweep in
            effects.add(new EffectEntry("sweep in", () -> Fx.sweepIn(Motion.LEFT_TO_RIGHT, 30, 0,
                    screenBg, slow, Interpolation.QuadOut)));

            // Smooth expand and reversed
            effects.add(new EffectEntry("smooth expand and reversed",
                    () -> Fx.sequence(
                            Fx.expand(ExpandDirection.VERTICAL, Style.EMPTY.fg(bg).bg(screenBg),
                                    1200, Interpolation.QuadOut),
                            Fx.expand(ExpandDirection.HORIZONTAL, Style.EMPTY.fg(bg).bg(screenBg),
                                    1200, Interpolation.QuadOut).reversed())));

            // Irregular sweep out/sweep in
            effects.add(new EffectEntry("irregular sweep out/sweep in", () -> Fx.sequence(
            ...
            // Coalesce
            effects.add(new EffectEntry("coalesce", () -> Fx.sequence(
                    Fx.coalesce(medium, Interpolation.CubicOut),
                    Fx.dissolveTo(Style.EMPTY.bg(screenBg), medium, Interpolation.QuadOut))));

            // Slide in/out
            effects.add(new EffectEntry("slide in/out",
                    () -> Fx.sequence(
                            Fx.parallel(Fx.fadeFromFg(bg, 2000, Interpolation.ExpoInOut),
                                    Fx.slideIn(Motion.UP_TO_DOWN, 20, 0, DARK0_HARD, medium,
                                            Interpolation.Linear)),
                            Fx.slideOut(Motion.LEFT_TO_RIGHT, 80, 0, DARK0_HARD, medium,
                                    Interpolation.Linear))));
            ...
            effects.add(new EffectEntry("color transitions",
                    () -> Fx.sequence(
            ...
            // In Rust version, this has CellFilter::FgColor(Light3), but we'll apply to all
            // for simplicity
            effects.add(new EffectEntry("color cycle",
                    () -> Fx.sequence(
