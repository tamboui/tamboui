 ...
 * <p>
 * Left panel: List of available effects (navigate with Up/Down arrows) Center
 * panel: Content area showing various text and widgets
 * <p>
 * Controls: - Up/Down arrows: Navigate effect list - Enter/Space: Trigger
 * selected effect - 't': Toggle between text and colored blocks view - 'q':
 * Quit
 */
public class BasicEffectsTFXDemo {
    ...
    private void run() throws Exception {
        ...
        TuiConfig config = TuiConfig.builder().tickRate(java.time.Duration.ofMillis(16)) // ~60fps
                .build();
     ...
     * @param startIdx
     *            The starting index
     * @param itemToDemoIndex
     *            The mapping from item index to demo index
     * @param direction
     *            True for forward (down), false for backward (up)
    ...
    /**
     * Finds the next selectable index (searches forward first, then backward). This
     * is used as a fallback when we don't know the direction.
     */
    private int findNextSelectableIndex(int startIdx, List<Integer> itemToDemoIndex) {
    ...
    private void triggerSelectedEffect() {
        if (selectedEffectIndex != null && selectedEffectIndex >= 0
                && selectedEffectIndex < demos.size()) {
            effectManager = new EffectManager();
    ...
    private void initializeDemos() {
        ...
        demos.add(new EffectDemo("Fade To (RGB)", "Fade",
                () -> Fx.fadeToFg(Color.CYAN, 2000, Interpolation.SineInOut)
                        .withFilter(CellFilter.text()).withColorSpace(TFxColorSpace.RGB)));

        demos.add(new EffectDemo("Fade To (HSL)", "Fade",
                () -> Fx.fadeToFg(Color.CYAN, 2000, Interpolation.SineInOut)
                        .withFilter(CellFilter.text()).withColorSpace(TFxColorSpace.HSL)));
        ...
        demos.add(new EffectDemo("Dissolve", "Dissolve",
                () -> Fx.dissolve(2000, Interpolation.Linear).withFilter(CellFilter.text())));
        ...
        demos.add(new EffectDemo("Coalesce", "Dissolve",
                () -> Fx.coalesce(2000, Interpolation.QuadIn).withFilter(CellFilter.text())));

        // Sweep effects (no filter - affects all cells including backgrounds)
        demos.add(new EffectDemo("Sweep In (L→R)", "Sweep", () -> Fx.sweepIn(Motion.LEFT_TO_RIGHT,
                10, 0, Color.BLUE, 2000, Interpolation.QuadOut)));

        demos.add(new EffectDemo("Sweep In (R→L)", "Sweep", () -> Fx.sweepIn(Motion.RIGHT_TO_LEFT,
                10, 0, Color.GREEN, 2000, Interpolation.QuadOut)));

        demos.add(new EffectDemo("Sweep In (U→D)", "Sweep", () -> Fx.sweepIn(Motion.UP_TO_DOWN, 10,
                0, Color.MAGENTA, 2000, Interpolation.QuadOut)));

        demos.add(new EffectDemo("Sweep Out (D→U)", "Sweep", () -> Fx.sweepOut(Motion.DOWN_TO_UP,
                10, 0, Color.YELLOW, 2000, Interpolation.QuadOut)));

        // Paint effects
        demos.add(new EffectDemo("Paint FG", "Paint", () -> Fx
                .paintFg(Color.RED, 1500, Interpolation.SineInOut).withFilter(CellFilter.text())));

        demos.add(new EffectDemo("Paint BG", "Paint", () -> Fx
                .paintBg(Color.BLUE, 1500, Interpolation.SineInOut).withFilter(CellFilter.text())));
        ...
        demos.add(
                new EffectDemo("Fade + Diagonal", "Patterns",
                        () -> Fx.fadeToFg(Color.MAGENTA, 2000, Interpolation.SineInOut).withPattern(
                                DiagonalPattern.topLeftToBottomRight().withTransitionWidth(5.0f))
                                .withFilter(CellFilter.text())));
        ...
        demos.add(new EffectDemo("Sequence", "Composition",
                () -> Fx.sequence(
                        Fx.fadeFromFg(Color.CYAN, 1500, Interpolation.SineInOut)
                                .withFilter(CellFilter.text()),
                        Fx.dissolve(2000, Interpolation.QuadOut).withFilter(CellFilter.text()))));

        demos.add(new EffectDemo("Parallel", "Composition",
                () -> Fx.parallel(
                        Fx.fadeToFg(Color.CYAN, 2000, Interpolation.SineInOut)
                                .withFilter(CellFilter.text()),
                        Fx.dissolve(2500, Interpolation.QuadOut).withFilter(CellFilter.text()))));
    ...
    private void render(Frame frame) {
        ...
        Layout layout = Layout.horizontal().constraints(Constraint.percentage(35), // Left panel:
                                                                                   // 35%
    ...
    private void renderEffectList(Frame frame, Rect area) {
        ...
        for (int i = 0; i < demos.size(); i++) {
            ...
            if (currentCategory == null || !currentCategory.equals(demo.category)) {
                ...
                items.add(ListItem
                        .from(Text.from(Line.from(Span.raw("── " + demo.category + " ──").dim()))));
        ...
        ListWidget list = ListWidget.builder().items(items)
                .highlightStyle(Style.EMPTY.fg(Color.YELLOW).bold()).highlightSymbol("▶ ")
                .block(Block.builder().borders(Borders.ALL).title(Title.from(
                        Line.from(Span.raw("Effects "), Span.raw("(" + demos.size() + ")").dim())))
                        .build())
                .build();
    ...
    private void renderContentArea(Frame frame, Rect area) {
        Block block = Block.builder().borders(Borders.ALL).title(Title.from(Line.from(Span
                .raw(showColoredBlocks ? "Content Area (Colored Blocks)" : "Content Area (Text)"))))
                .build();
    ...
    private void renderTextContent(Frame frame, Rect area) {
        ...
        content.append(
                "• A longer line of text that demonstrates how effects work across different line lengths\n");
        ...
        Paragraph paragraph = Paragraph.builder().text(Text.from(content.toString()))
                .alignment(Alignment.LEFT).build();
    ...
    private void renderColoredBlocks(Frame frame, Rect area) {
        ...
        for (int y = area.top(); y < area.bottom(); y += blockHeight) {
            for (int x = area.left(); x < area.right(); x += blockWidth) {
                ...
                // Create a block area
                Rect blockArea = new Rect(x, y, Math.min(blockWidth, area.right() - x),
                        Math.min(blockHeight, area.bottom() - y));

                // Fill with colored cell (using full block character for solid color)
                Cell coloredCell = new Cell("X", Style.EMPTY.bg(bgColor));
        ...
        if (area.height() > 2) {
            ...
            if (infoX >= area.left() && infoX + info.length() <= area.right()) {
                frame.buffer().setString(infoX, infoY, info, Style.EMPTY.fg(Color.WHITE).bold());
            }
        }
    }
}
